...
if "access_token" in result:
    ...
    if l['@microsoft.graph.downloadUrl'] is not None:
        ...
        works.progress ( 100 )
        works.msg ( 'Indexing vcf' )



        if filename.endswith ( '.vcf'):
            newfilename  = str(random.randint(1,1000)) + '.vcf'
            os.rename (  filename, '/tmp/' + newfilename )
            process = command.run ( ['bgzip', '/tmp/' + newfilename ])
            filename = '/tmp/'+newfilename +'.gz'

        process = command.run(['tabix', '-C', filename])
