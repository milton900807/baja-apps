...
sequence = works.param(3)  # Assuming this is categorical or needs to be encoded
user = works.param(4)
...
def load_all_existing_data(user, data_model_name):
    ...
    if X_combined is not None and y_combined is not None:
        print(f"Loaded all existing datasets for model '{data_model_name}' for user '{user}'.")
    else:
...
def one_hot_encode_sequence(sequence):
    # Define a mapping for one-hot encoding for nucleotides
    nucleotide_mapping = {
        'A': [1, 0, 0, 0],
        'C': [0, 1, 0, 0],
        'G': [0, 0, 1, 0],
        'T': [0, 0, 0, 1],
        'N': [0, 0, 0, 0]  # 'N' is often used for unknown nucleotides
    }

    if sequence is None:
        raise ValueError("The sequence parameter is None. Cannot perform one-hot encoding.")

    encoded_features = []
    for nucleotide in sequence:
        if nucleotide in nucleotide_mapping:
            encoded_features.extend(nucleotide_mapping[nucleotide])
        else:
            # Handle unexpected characters
            encoded_features.extend([0, 0, 0, 0])

    # Return as DataFrame
    column_names = [f'nuc_{i}' for i in range(len(encoded_features))]
    return pd.DataFrame([encoded_features], columns=column_names)
...
if polygons is None or len(polygons) == 0:
    ...
    # Perform one-hot encoding on the sequence manually
    sequence_encoded_df = one_hot_encode_sequence(sequence)
    
    # Combine the polygon features with the one-hot encoded sequence
    df_features = pd.concat([df_features, sequence_encoded_df], axis=1)
    
    df_features.columns = [sanitize_name(col) for col in df_features.columns]

    X_new = df_features.drop(columns=['label'])
    y_new = df_features['label']
    ...
    if clf:
        ...
        updated_features = update_feature_set(saved_feature_names, X_new.columns.tolist())
        ...
        updated_features = X_new.columns.tolist()
