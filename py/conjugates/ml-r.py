...
def linearKd  ( df, platform, variable, order, outputdir, prefix ):
    ...
    SAVEit = False
    if count > 10 and mse < 10 and r2 > 0.5:
        SAVEit = True
    if SAVEit: 
        description = """
	The model is a Gradient Boosting Regressor trained on polynomial features of the input data. 
	...
	# Replace N with the actual degree and M with the number of estimators
        ...
        description = description.replace("M", str(100))  # Example: 100 estimators
        #for i, tree in enumerate(model.estimators_):
        #    tree_rules = export_text(tree[0], feature_names=[f"poly_feature_{i}" for i in range(X_poly.shape[1])])
        #    description += "\n" + tree_rules
        feature_importances = model.feature_importances_
...
t = chemfeatures  + chemical_functional_groups
def chem_features():
    return random.sample(t, 3)

unique_unigenes = df['Unigene'].unique().tolist()
variable = chem_features (  )


 #7count_WW_domainsketone_DRD1
 #Count: 32
 #Mean Squared Error (MSE): 6.55
 #R-squared (R2): -0.32
 #['count_WW_domains']
 #['TPSA']
 #  7count_WW_domainsTPSA_DRD1
sub_df = df[df['Unigene'] == 'DRD1']
linearKd(sub_df, ['count_WW_domains'], ['TPSA'], 7, './output', 'verify-output')

sub_df = df[df['Unigene'] == 'HTR4']
linearKd(sub_df,['count_helix_loop_helix'], ['hydrophilic_in_acidic_conditions', 'phenyl', 'Hydrophobicity'], 2, './output', 'verify2') 

for i in range(1,len(unique_unigenes)):
    sub_df = df[df['Unigene'] == unique_unigenes[i]]
    for order in range (1,7):
        for m in method_names:
            for v in variable: 
                linearKd ( sub_df, [m], [v], order, './output', f'{i}{m}{v}_{unique_unigenes[i]}')
            linearKd ( sub_df, [m], variable, order, './output', f'{i}{m}_{unique_unigenes[i]}')
