...
def linearKd  ( df, platform, variable, order, outputdir, prefix ):
    ...
    # lmodel = LinearRegression(n_estimators=1000, learning_rate=0.1, max_depth=3, random_state=42)
...
def testModel  ( df, platform, variable, order, testset ):
    print ( f'{platform} ')
    print ( f'{variable} ')
    print ( f' ^{order}')
    interaction_df = pd.DataFrame(index=df.index)
...
dbv = db.loadProsite('../prosite/prosite.db')

sequences_unique = df['protein_sequence'].unique()
columns = ['ID', 'LOC', 'NUM', 'DES']
sp = pd.DataFrame(columns=['ID', 'LOC', 'NUM', 'DES'])

# for s in sequences_unique:
#     r = db.find_pattern_hits ( s, dbv)
#     for i in range (0,len(r)):
#         print ( r[i]['ID'] )
#     df_to_append = pd.DataFrame(r)    
#     sp = pd.concat([sp, df_to_append], ignore_index=True)
 
# sp.to_csv ( 'prosite-seq.csv')


for _, __row in dbv.iterrows():
    print ( __row['ID'] )
    hits_series = df.apply(lambda row: db.find_pattern_hits_from_id(row['protein_sequence'], dbv, __row['ID']), axis=1)
    if any(len(hits) > 0 for hits in hits_series):
            df[__row['ID']] = hits_series
