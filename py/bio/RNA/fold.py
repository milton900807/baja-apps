...
# NEED TO ISNTALL VIENNA RNA AND FORGI 




def mfe_struct(sequence):
        try:
            result = RNA.fold_compound(sequence)
            return result.mfe()[0]
        except Exception as ex:
            print ( ' Failed to generate the mfe_structure ')
        return None

def struct_positions(sequence, struc):
    header = ' Test object '
    try:
        #fasta_text = ">{}\n{}\n{}".format(header, sequence, struc)
        fasta_text = r"> \n " + sequence + " \n " + struc
        result = fasta_to_positions(fasta_text)
        return result
    except Exception as ex:
...
def fasta_to_positions(fasta_text):
    print ( ' ------------------------------------------- \n')
    bg = fgb.BulgeGraph.from_fasta_text(fasta_text)
    print ( ' ------------------------------------------- \n')
    bp_string = bg[0].to_dotbracket_string()
    print ( ' ------------------------------------------- \n')
...
try:
    #sequence =  'AUCGUACGAUCGUAGCUCGUAGCUAGCAUCGUACGAUCGUAGCUAGCAUCGUACGAUCGUAGCUAGCAUCGUACGAUCGUAGCUAGCAUCGUACGAUCGUAGCUAGC'
    sequence = str(works.param(1))
    md = RNA.md()
    ...
    print (s)
    print ( pt )
    result = struct_positions ( sequence, s )
    list_of_tuples = list(result)
    works.resolve(list_of_tuples)
except Exception as e: 
    print(f"An error occurred: {e}")
    works.resolve ( json.dumps ( e ))
