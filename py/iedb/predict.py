...
# Define valid amino acids and their approximate charge properties at physiological pH
...
def compute_charge_features(peptides):
    """Extracts hydrophobicity, polarity, and molecular weight features per peptide (not per amino acid)."""
    ...
    for peptide in peptides:
        ...
        # Aggregate per peptide instead of per amino acid
        features = [
            np.mean(hydro_values), np.std(hydro_values),  # Hydrophobicity
            np.mean(polar_values), np.std(polar_values),  # Polarity
            np.mean(mw_values), np.std(mw_values),        # Molecular Weight
            len(peptide)  # Peptide length as a feature
        ]
...
joblib.dump(model, "epitope_model.pkl")
predict_epitope_scores("../../epitope_full_v3.xlsx", "epitope_model.pkl")
