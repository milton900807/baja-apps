...
def extract_valid_peptides(file_path):
    df = pd.read_excel(file_path, usecols=["Name"])  # Read only the "Name" column
    peptide_column = df["Name"].dropna().astype(str).str.strip()  # Clean data
    return peptide_column[peptide_column.apply(is_valid_peptide)].tolist()  # Filter valid peptides
