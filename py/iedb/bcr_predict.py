...
# Load pre-trained deep learning model
model = tf.keras.models.load_model("tcr_model.h5")
def fetch_phylogeny_distance(uniprot_id):
    """Fetch phylogenetic distance from UniProt."""
    url = f"https://rest.uniprot.org/uniprotkb/{uniprot_id}.txt"
    try:
        response = requests.get(url, timeout=10)
        response.raise_for_status()
        ...
        for line in content.split("\n"):
            if line.startswith("OX"):
                parts = line.split("=")
                if len(parts) > 1 and parts[1].strip().isdigit():  # Ensure it's a number
                    return int(parts[1].strip())  # Return numerical taxonomy ID
        return None  # Return None if not found or incorrect format
    except requests.RequestException:
        return None
...
def process_uniprot_ids(csv_file):
    ...
    for index, row in df.iterrows():
        ...
        if protein_sequence:
            peptide_windows = generate_peptide_windows(protein_sequence)
            high_epitope_count, _ = predict_epitopes(peptide_windows, model)
            phylogeny_distance = fetch_phylogeny_distance(uniprot_id)
            results.append((uniprot_id, high_epitope_count, phylogeny_distance))
    
    results_df = pd.DataFrame(results, columns=["UniProt ID", "High Epitope Count", "Phylogeny Distance"])
    
    # Drop missing values
    results_df = results_df.dropna()
    
    # Compute correlation
    correlation = results_df["Phylogeny Distance"].corr(results_df["High Epitope Count"])
    print(f"Correlation between Phylogenetic Distance and High Epitope Count: {correlation:.3f}")
    
    # Plot
    plt.figure(figsize=(10, 6))
    plt.scatter(results_df["Phylogeny Distance"], results_df["High Epitope Count"], alpha=0.7)
    plt.xlabel("Phylogenetic Distance")
    plt.ylabel("High Epitope Count")
    plt.title("Correlation between Phylogenetic Distance and High Epitope Count")
    plt.grid()
    plt.savefig("epitope_phylogeny_distance.png")
    print("Plot saved as epitope_phylogeny_distance.png")
    
    return results_df

# Example usage:
csv_file = "../insulin.csv"
