...
for day_count in ndays:
    ...
    for lineage in voc:
        for location in loc:
            url = '/genomics/prevalence-by-location?pangolin_lineage={lineage}&location_id={location}'
...
report = df.groupby(['lineage', 'location', 'ndays']).mean()
temp_dir_path = works.tempdir ()
report.to_excel ( temp_dir_path + '/linage_report.xlsx')

ress['images'] = res;
ress['downloads'] = [{'file_url':temp_dir_path + '/linage_report.xlsx'}]
