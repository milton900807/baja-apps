...
def generate_domain_block_and_anchor_hints(
    ...
    *,
    ...
    user_msg = (
        ...
        "- Refer to many tables where each has more than 20 rows and many formulas.\n"
        "- At least 126 tables total; at least 240 distinct formulas can be constructed from these anchors.\n"
...
- Provide AT LEAST 8 distinct and unique tables.
- All labels are unique
- Provide AT LEAST 20 distinct and unique labeled rows across all tables (excluding header rows).
- Provide AT LEAST 40 unique formulas total (in "formulas"), and at least 8 formulas per table.
...
- Return ONLY a JSON object with exactly four top-level keys: "tables", "formulas", "formula_annotations", "annotations", "units". No prose.
...
- Must have unique formulas that calculate revenue, operating costs, monthly-operating costs and annual opreating costs and captial required if doing a startup. "

- alphanumeric characters only  in names and labels
- All tables have 2 cols; [0:0][0:0]='Label', [1:1][0:0]='Value'
- Label col must be fully labeled and each is label is unique; column two will have unique formulas reference table[Label] (no quotes)
...
FORMULA RULES
- do not use range queries e.g. [1:1][0:0] in formulas values i.e. right side you can use it to assign
- References use labels only: other_table[Field_Label]. No ranges. No dot notation.
...
- Formulas user fully qualified references tablename[field1,field2] etc.
...
- labels are unique and NOT null or empty strings and only alphabet characters
...
"Profit_and_Loss[6:6][0:0]": "Profit_and_Loss[5:5]/Revenue[Projection]",
"Profit_and_Loss[7:7][0:0]": "Profit_and_Loss[6:6]-Profit_and_Loss[9:9]",
"Profit_and_Loss[1:1][2:2]": "Revenue[Value]-COGS[Total_COGS]",
"Profit_and_Loss[1:1][3:3]": "Profit_and_Loss[2:2]-Operating_Expenses[Value]",
"Profit_and_Loss[1:1][4:4]": "Profit_and_Loss[5:5]-Taxes[Total_Taxes]",
"Profit_and_Loss[1:1][10:10]": "Operating_Expenses[Value]/12",
"Profit_and_Loss[1:1][11:11]": "Capex[Total_Capex]/Assumptions[Average_Asset_Life]",
"Profit_and_Loss[1:1][12:12]": "Assumptions[Target_Runway_Months]*Operating_Expenses[Value]/Net_Income",
"Profit_and_Loss[1:1][13:13]": "Revenue[Value]-COGS[Total_COGS]-Operating_Expenses[Value]-Depreciation[Annual_Depreciation]",
"Revenue[1:1][1:1]": "Assumptions[Services_Revenue]",
"COGS[1:1][1:1]": "Revenue[Projection]*Assumptions[COGS_Percentage]",
"Operating_Expenses[1:1][1:1]": "Operating_Expenses[Salary]+Operating_Expenses[Rent]+Operating_Expenses[Utilities]",
"Hiring_FTE[1:1][1:1]": "Hiring_FTE[Biologist]+Hiring_FTE[Chemist_2]+Hiring_FTE[Engineer_2]",
"Compensation[1:1][1:1]": "Hiring_FTE[Biologist]*Assumptions[Avg_Salary]+Hiring_FTE[Chemist_2]*Assumptions[Avg_Salary]+Hiring_FTE[Engineer_2]*Assumptions[Avg_Salary]",
"Capex[1:1][1:1]": "Assumptions[Capex_Rate]*Assumptions[Capex_Year_1]",
"Depreciation[1:1][1:1]": "Capex[Total_Capex]/Assumptions[Depreciation_Years]",
"Working_Capital[1:1][1:1]": "Working_Capital[Current_Assets]-Working_Capital[Current_Liabilities]",
"Taxes[1:1][1:1]": "(Profit_and_Loss[Net_Income]-Operating_Expenses[Value]-Depreciation[Annual_Depreciation])*Assumptions[Tax_Rate]",
"Cash_Flow[1:1][1:1]": "Cash_Flow[Inflows]-Cash_Flow[Outflows]",
"Cash_Flow[1:1][2:2]": "Cash_Flow[Inflows]-Cash_Flow[Outflows]",
"KPIs[1:1][1:1]": "IF(YearwhenProfit_and_Loss[Net_Income]>=0,TRUE,FALSE)",
"Outputs[1:1][1:1]": "Profit_and_Loss[Net_Income]-Taxes[Total_Taxes]",
"Scenarios[1:1][1:1]": "Revenue[Value]*1",
"YearwhenProfit_and_Loss[1:1][1:1]": "YearwhenProfit_and_Loss[Net_Income]"
...
def expand_user_prompt(
    ...
    *,
    ...
    user_msg = (
        ...
        + "Expand this to describe more details and provides an explicit list of at least 100 tables and fomrulas the model should try to capture; for example KPI, capital required, operating costs per month for 3 years, and a table with FTE broken down by key positions and relevant salaries etc. "
    ...
    *,
    ...
    try:
        ...
        user_msg = (
            ...
            "- All cells in the first column will have a uniique label that describes the unique formula in the second column.\n"
            ...
            "- Optional expansion: up to 4 logical tables and ~24 unique formulas that integrate with existing labels.\n"
...
def run_two_stage(
    ...
    *,
    ...
    previous_results: Optional[dict] = None,
    max_first_pass_retries: int = 4,   # <-- configurable retries
) -> Dict[str, Any]:
    """
    1) Expand the prompt (include summary of prior model if provided)
    2) Build JSON via scaffold (with retry if formulas are missing)
    ...
    expanded = expand_user_prompt(
        user_prompt, model=model, temperature=1.0, previous_results=previous_results
    )

    # Also append a compact prior summary directly into the user content for the scaffold phase
    if previous_results:
        expanded += "\n\n(Use and extend the following prior model context where helpful.)\n"
        expanded += _summarize_prior_for_prompt(previous_results)

    if _HAS_ION:
        works.msg("Building model...")

    # --- First pass with retry if no formulas ---
    attempt = 0
    first = None
    cur_temp = 0.62  # start conservative for structure
    while True:
        attempt += 1
        ...
        out = first.get("output", {}) or {}
        has_formulas = bool((out.get("formulas") or {}))

        if has_formulas:
            if _HAS_ION:
                works.msg(f"Model complete on attempt {attempt}")
            break

        if attempt > max_first_pass_retries:
            if _HAS_ION:
                works.msg(f"No formulas after {attempt} attempts; continuing with best effort.")
            break
        # cur_temp += 0.4
        if _HAS_ION:
            works.msg(f"No formulas returned; retrying first pass (attempt {attempt+1}, temp={cur_temp})")
