...
specialized for GMP ASO / GalNAc programs with CDMOs.

What it does
------------
• Calls GPT to infer sensible INITIAL CAPITAL and WORKING CAPITAL assumptions from a user prompt.
• Designed for an asset-light / virtual therapeutics company running GMP ASO / GalNAc
  manufacturing via external CDMOs (no internal GMP plant, no lab buildout, no instruments).
• Capital is primarily used to fund:
    - GMP batches (ASOs, GalNAc conjugates),
    - release analytics and method development,
    - stability and forced degradation packages,
    - CDMO prepayments, deposits, and cash buffers.
• ALWAYS includes an Initial_Capital_Investment_USD assumption.
• If a Capital_per_ASO_USD label is present, it MUST be exactly 1,000,000 USD.
...
# Required labels + defaults
REQUIRED_INITIAL_LABEL = "Initial_Capital_Investment_USD"
DEFAULT_INITIAL_CAPITAL_VALUE = 5_000_000  # can be tuned later

# Hard constraint: Capital_per_ASO_USD must be exactly 1,000,000 if present
CAPITAL_PER_ASO_LABEL = "Capital_per_ASO_USD"
CAPITAL_PER_ASO_FIXED_VALUE = 1_000_000

# ---------- LLM prompt for capital & working capital assumptions ----------
CAPITAL_JSON_INSTRUCTIONS = f"""
You generate ONLY a two-column 'Capital_Assumptions' table for a financial/operational model.

Context:
- The organization is an asset-light / virtual therapeutics company.
- GMP manufacturing of antisense oligonucleotides (ASOs) and GalNAc conjugates is fully
  or largely outsourced to CDMOs (no internal GMP plant, no internal lab buildout).
- There is NO internal lab buildout, NO equipment purchases, and NO instruments.
- Capital is primarily used to fund:
    • GMP batches (ASO / GalNAc),
    • release analytics & method development,
    • stability and forced degradation packages,
    • CDMO prepayments, deposits, and cash buffer,
    • general working capital runway.
...
- If you include a label named "Capital_per_ASO_USD", its value MUST be exactly {CAPITAL_PER_ASO_FIXED_VALUE}.
  Do NOT use any other number for Capital_per_ASO_USD.

Your goal is to:
1) MINIMIZE the number of different capital "types" (concepts),
2) But ALLOCATE that total initial capital across the concrete GMP ASO needs implied by the user prompt
   (e.g., number of ASO programs, number of GMP batches, grams required, stability packages).

If the user prompt clearly implies N assets (for example, "20 ASO compounds", "5 programs",
"3 lead indications", "4 GMP batches", "10 constructs"), then you MUST:
- Infer that N as an integer.
- Include a row that captures the asset/batch count, e.g.:
    • ASO_Compounds_Count
    • Programs_Count
    • GMP_Batches_Count
    • Assets_Count
...
- Allocate the Initial_Capital_Investment_USD across those assets using simple logic:
    • A per-unit capital row, e.g. Capital_per_ASO_USD, Capital_per_Program_USD,
      Capital_per_GMP_Batch_USD, or Capital_per_Asset_USD.
    • If N is small (≤ 20), you MAY also include one row per asset like:
        - ASO_01_Capital_USD
        - ASO_02_Capital_USD
      but keep the per-asset pattern consistent.
- Ensure that:
    • Initial_Capital_Investment_USD ≈ Count * Capital_per_Unit_USD
      (allow small rounding differences).
    • If the per-unit label is Capital_per_ASO_USD, it MUST be exactly {CAPITAL_PER_ASO_FIXED_VALUE}.
...
- ASO_Compounds_Count / Programs_Count / GMP_Batches_Count / Assets_Count
- Capital_per_ASO_USD / Capital_per_Program_USD / Capital_per_GMP_Batch_USD
- Working_Capital_Months_of_Runway
- Minimum_Cash_Reserve_USD
- Monthly_Burn_Estimate_USD
- CDMO_Prepayment_Working_Capital_USD (optional)
- Operating_Cash_Buffer_USD (optional)
- Stability_Package_Capital_USD (optional)

Return STRICT JSON with this EXACT schema:
{{
  "capital_assumptions": [
    {{"label": "Initial_Capital_Investment_USD", "value": 5000000}},
    {{"label": "ASO_Compounds_Count", "value": 20}},
    {{"label": "Capital_per_ASO_USD", "value": {CAPITAL_PER_ASO_FIXED_VALUE}}},
...
- Include **5–15** rows, focusing on:
    • total initial capital,
    • allocation across ASO/GalNAc assets, programs, or GMP batches implied by the prompt,
...
- CDMO / vendor working capital items (no internal lab):
    • CDMO_Prepayment_Working_Capital_USD (optional)
...
def generate_capital_assumptions(
    ...
    *,
    ...
    system = (
        ...
        "You strictly focus on INITIAL CAPITAL and WORKING CAPITAL for an asset-light / virtual GMP ASO / GalNAc company, "
        "with NO lab buildout, NO equipment, and NO instruments. "
        "You MUST always include Initial_Capital_Investment_USD. "
        "If you emit Capital_per_ASO_USD, it MUST equal 1000000. "
        "You STRICTLY follow output schemas."
    )
    user = f"{CAPITAL_JSON_INSTRUCTIONS}\n\nUser prompt:\n{user_prompt.strip() if user_prompt else ''}"

    works.msg(
        "🏗️ requesting Capital_Assumptions JSON from GPT "
        "(initial & working capital for GMP ASO / GalNAc, no lab/equipment/instruments)…"
    )
    content = _chat_call(
        model=model,
        system=system,
    ...
    for it in items:
        ...
        # Enforce hard constraint on Capital_per_ASO_USD
        if label == CAPITAL_PER_ASO_LABEL:
            value = CAPITAL_PER_ASO_FIXED_VALUE
        else:
            value = it.get("value", "")
...
def infer_units(table_name: str, rows: List[Dict[str, str]]) -> Dict[str, Dict[str, str]]:
    ...
    for kv in rows:
        ...
        elif any(x in l for x in ["cost", "investment", "raise", "balance", "buffer", "deposit", "prepayment", "fees", "reserve", "capital"]):
            units[table_name][lab] = "USD"
        elif "burn" in l:
            units[table_name][lab] = "USD/month"
        elif "batches_count" in l or "compounds_count" in l or "assets_count" in l or "programs_count" in l:
...
def run_capital_assumptions_only(
    ...
    *,
    ...
    works.msg(
        "💡 capital & working-capital assumptions-only pipeline starting "
        "(GMP ASO / GalNAc, no lab/equipment/instruments; Capital_per_ASO_USD fixed at 1,000,000)…"
    )
    ...
    artifact: Dict[str, Any] = {
        ...
        "annotations": {
            "Capital_Assumptions": (
                ...
                "for initial capital and working capital for GMP ASO / GalNAc, "
                "with NO lab/equipment/instruments, guaranteed Initial_Capital_Investment_USD, "
                "and Capital_per_ASO_USD forced to 1,000,000 if present."
            )
        },
...
if __name__ == "__main__":
    works.msg(
        "🔧 loading capital & working-capital assumptions-only builder "
