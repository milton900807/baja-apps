...
• Applies deterministic logic for complex procedures AND number of compounds:
      - If the prompt and/or route of administration implies neuro-targeted
        ...
        CNS / neuron-heavy work, or other brain/spinal administration, the script:
      ...
      - If not neuro / highly complex, Complex_Procedure_Flag = 0, multiplier = 1.0
      - Independently, the script tries to infer Number_of_Compounds from the user prompt
        (e.g., "3 compounds", "five candidates") and sets/overrides Number_of_Compounds
        accordingly, falling back to assumptions or default if not stated.
...
RODENT_TOX_SCAFFOLD = [
    ...
    "Number_of_Compounds",                  # number of compounds / test articles in study
...
Key rate- and cost-limiting steps:
   - Complex surgery / neuro-targeted procedures (ICV, IT, neuron / CNS)
...
- Focus on OPERATIONS and STUDY DESIGN for CRO rodent tox:
  * Species, Strain, Number_of_Compounds, Number_of_Animals_per_Group,
    Number_of_Dose_Groups, Study_Type.
...
# ---------- Prompt-driven inference helpers (compounds, complexity) ----------

def _infer_compounds_from_prompt(user_prompt: str) -> Optional[int]:
    """
    Tries to infer the number of compounds from free text.

    Matches:
      - "3 compounds", "3-compound panel"
      - "4 molecules", "4 candidates", "4 assets"
      - "2 test articles", "2 test items"
      - "three compounds", "two candidates", etc.

    Returns:
      Integer count, or None if nothing obvious is found.
    """
    if not user_prompt:
        return None
    ...
    # 1) Digit-based mentions near compound-like words
    target_words = r"(compound|compounds|molecule|molecules|candidate|candidates|asset|assets|test\s+article|test\s+articles|test\s+item|test\s+items)"
    digit_patterns = [
        rf"(\d+)\s*[- ]\s*{target_words}",   # 3-compound, 3 compound
        rf"(\d+)\s+{target_words}",          # 3 compounds
    ]
    ...
    # 2) Number words like "three compounds"
    number_word_map = {
        "one": 1,
        "two": 2,
        "three": 3,
    ...
    word_list = "|".join(number_word_map.keys())
    word_pattern = rf"\b({word_list})\s+{target_words}"
    m2 = re.search(word_pattern, text)
    if m2:
        word = m2.group(1)
        val = number_word_map.get(word)
        if val and val > 0:
            return val

    # 3) Edge case: "N-compound" without extra context
    m3 = re.search(r"(\d+)\s*-\s*compound", text)
    if m3:
        try:
            val = int(m3.group(1))
            if val > 0:
...
def _infer_neuro_complexity_from_prompt_and_route(user_prompt: str, route_raw: str) -> bool:
    """
    Heuristically decide whether the study is "neuro / highly complex",
    based on neurons / CNS / ICV / IT / brain / spinal-like wording.

    Returns:
      True if complex, False otherwise.
    """
    text = (user_prompt or "").lower()
    route = (route_raw or "").lower()
    ...
    neuro_keywords = [
        ...
        "intrathecal", " it injection", "it injection",
        "intracisternal",
        "ventricular injection",
    ]

    for kw in neuro_keywords:
        if kw in text or kw in route:
            return True
    return False
...
def generate_assumptions_from_taxonomy(
    *,
    ...
    # enforce neuro / complex procedure logic AND prompt-driven compound count
    out = _apply_complex_procedure_and_compound_logic(out, user_prompt=user_prompt)
...
# ---------- Complex procedure + compound count logic ----------

def _apply_complex_procedure_and_compound_logic(
    rows: List[Dict[str, str]],
    user_prompt: str
) -> List[Dict[str, str]]:
    """
    Detect complex neuro-targeted procedures (e.g., ICV, IT, neuron/CNS) and
    set a 2x cost multiplier, and also infer Number_of_Compounds from the prompt.

    Logic:
      - is_complex = True if neuron / CNS / ICV / IT / brain / spinal cues present
        in user_prompt or Route_of_Administration.
      - Complex_Procedure_Flag = 1 if complex, else 0
      - Complex_Procedure_Cost_Multiplier = 2.0 if complex, else 1.0
      - Multiply Dose_Admin_Cost_per_Animal_USD and Surgery_Cost_per_Animal_USD
        by the multiplier if complex.

      - Try to infer Number_of_Compounds from user_prompt. If found, set that value.
        If not found and Number_of_Compounds is missing, backfill from DEFAULTS.
    """
    ...
    def set_val(label: str, value: float | int | str) -> None:
    ...
    # ---- 1) Number_of_Compounds from prompt ------------------------------------
    inferred_compounds = _infer_compounds_from_prompt(user_prompt)
    if inferred_compounds is not None:
        set_val("Number_of_Compounds", inferred_compounds)
    else:
        existing = get_val("Number_of_Compounds")
        if existing is None or not str(existing).strip():
            default_compounds = DEFAULTS.get("Number_of_Compounds", "1")
            try:
                default_compounds_val = int(default_compounds)
            except Exception:
                default_compounds_val = 1
            set_val("Number_of_Compounds", default_compounds_val)

    # ---- 2) Complex procedure logic (ICV / IT / neuro / CNS) -------------------
    route_raw = get_val("Route_of_Administration") or ""
    is_complex = _infer_neuro_complexity_from_prompt_and_route(user_prompt, route_raw)
    ...
    if is_complex:
        for lab in ["Dose_Admin_Cost_per_Animal_USD", "Surgery_Cost_per_Animal_USD"]:
            val_str = get_val(lab)
            if val_str is None:
                continue
            try:
...
def infer_units(table_name: str, rows: List[Dict[str, str]]) -> Dict[str, Dict[str, str]]:
    ...
    for kv in rows:
        ...
        elif "number_of_compounds" in l:
            units[table_name][lab] = "count"
        elif "number_of_animals" in l:
            units[table_name][lab] = "animals"
    ...
    artifact: Dict[str, Any] = {
        ...
        "annotations": {
            TABLE_NAME: (
                ...
                "including study design parameters, number of compounds, per-animal "
                "procedure costs, TK and histopathology knobs, payment terms, and a "
                "complex procedure (e.g., ICV/IT/neuronal) multiplier that doubles "
                "key per-animal costs when neuro-targeted complexity is detected "
...
if __name__ == "__main__":
    works.msg(
        ...
        "(generalized, prompt-driven, with compound-count inference and complex "
