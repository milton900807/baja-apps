...
NOW returns: window.startDate and window.endDate
...
def _gpt_point_formula(catalog: Dict[str, Any], model: str, temperature: float) -> Optional[Dict[str, str]]:
    if not _HAS_OPENAI or not os.getenv("ANTHROPIC_API_KEY"):
        return None
...
# ----------------- TenYear fallback rows (unchanged core) -----------------
...
# ----------------- NEW: point formula assembly -----------------
def _gpt_point_formula(catalog: Dict[str, Any], model: str, temperature: float) -> Optional[Dict[str, str]]:
    # (defined above; kept here for readability in this block)
    return None  # shadowed by the earlier definition; real function is above
...
def build_ten_year_view_gpt(src_root: Any, years_ahead: int = 11, model: str = "claude-haiku-4-5", temperature: float = 0.15) -> Dict[str, Any]:
    ...
    # Ensure required rows exist; add growth-aware fallbacks if needed
    ...
    # NEW: also build the per-point y(x) formula & scaffolding
    ...
    annotations = {
        ...
        "Cash_vs_Time": "y(x) = cash-in-hand as a strict formula. Set Eval[x] = point.x_hours, then evaluate Cash_vs_Time[y].",
    }
