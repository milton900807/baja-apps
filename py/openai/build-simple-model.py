...
def _extract_json_snippet(text: str) -> str:
    ...
    # Scan for a balanced {...} while respecting strings and escapes
    start = text.find("{")
    if start == -1:
    ...
    # If we get here, we never closed the root object
...
REQUIRED Data in Assumptions TABLE (MANDATORY — must have at least tables reference via formula default values in Assumptions)
- FTE
- Compensation
- Capex
- Depreciation
...
- Each table should have more than ≥60 labeled rows, ≥120 formulas; ≥8 formulas per table.
...
def _fetch_gpt_defaults_for_missing_refs(
    *,
    ...
    try:
        ...
        # Coerce to strings to be consistent with existing table values convention
...
def _fetch_gpt_finance_seeds(domain_prompt: str, *, model: str = "gpt-4o-mini") -> Dict[str, Dict[str, str]]:
    ...
    user_msg = (
        ...
        "Depreciation, EBIT, Taxes, Net_Income, Burn_Rate."
    )
    try:
        content = _chat_call(
            model=model,
