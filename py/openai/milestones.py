...
- NEW: density param (0..1) to control result granularity (0=sparse, 1=dense).
- NEW: extracts financial params (starting cash, monthly burn, monthly revenue, gross margin, etc.)
       and computes revenue, expenses, P&L, and cash-on-hand at each milestone.
...
- No duplicates; sequence should be chronological where possible.
...
- Keep names concise and verb-focused. No duplicates.
- Valid JSON only; no commentary.
""".strip()
...
def infer_milestones_via_gpt(prompt: str, *, model: str, temperature: float, density: float) -> List[Dict[str, Any]]:
    ...
    instructions = _mk_infer_mst_instructions(target)
...
# ===== Financial extraction & computation =====
_FIN_RE = {
    "starting_cash": re.compile(r'(?i)\b(starting\s*cash|cash\s*on\s*hand|cash)\b[^$%\d]*\$?([\d,\.]+)'),
    "monthly_burn":  re.compile(r'(?i)\b(monthly\s*burn|burn\s*rate|burn)\b[^$%\d]*\$?([\d,\.]+)'),
    "monthly_rev":   re.compile(r'(?i)\b(monthly\s*rev(?:enue)?|revenue)\b[^$%\d]*\$?([\d,\.]+)'),
    "gross_margin":  re.compile(r'(?i)\b(gross\s*margin|gm|margin)\b[^%\d]*([0-9]{1,3})\s*%'),
}

def _extract_financials_regex(prompt: str) -> Dict[str, Any]:
    out: Dict[str, Any] = {}
    for key, rx in _FIN_RE.items():
        m = rx.search(prompt or "")
        if m:
            val = _to_number(m.group(2))
            if val is not None:
                if key == "gross_margin":
                    out[key] = max(0.0, min(1.0, val / 100.0))
                else:
                    out[key] = float(val)
    return out

def _mk_financial_instructions() -> str:
    return """
Return STRICT JSON:
{"financials":{
  "starting_cash": <number or 0>,
  "monthly_burn": <number or 0>,            // operating expenses per month (excl. COGS)
  "monthly_rev": <number or 0>,             // revenue per month
  "gross_margin": <0..1 or null>,           // optional; fraction (e.g., 0.65)
  "one_offs": [{"name":"<desc>","date":"YYYY-MM-DDTHH:MM:SS","amount": <number>}]
}}
Rules:
- Read the user's text and extract any explicit values. If absent, infer simple, plausible defaults ONLY if clearly implied; otherwise use 0.
- gross_margin is a fraction 0..1 (not percent). If unknown, set to null.
- one_offs should include cash events like funding, big purchases, prepayments if clearly present.
- Valid JSON only; no commentary.
""".strip()

def _extract_financials_gpt(prompt: str, *, model: str, temperature: float) -> Dict[str, Any]:
    try:
        raw = _chat_call(
            model=model,
            system="You extract basic financial parameters as JSON.",
            user=f"{_mk_financial_instructions()}\n\nUser prompt:\n{_sanitize_for_gpt_urls(prompt)}",
            temperature=temperature,
            json_mode=True,
            max_tokens=800
        )
        ...
        # normalize
        res = {
            "starting_cash": float(fin.get("starting_cash") or 0.0),
            "monthly_burn":  float(fin.get("monthly_burn") or 0.0),
            "monthly_rev":   float(fin.get("monthly_rev") or 0.0),
            "gross_margin":  fin.get("gross_margin"),
            "one_offs":      fin.get("one_offs") or []
        }
        # coerce one_offs dates
        clean = []
        for ev in res["one_offs"]:
            try:
                clean.append({
                    "name": str(ev.get("name") or "").strip() or "one-off",
                    "date": datetime.fromisoformat(str(ev.get("date"))),
                    "amount": float(ev.get("amount") or 0.0)
                })
            except Exception:
                continue
        res["one_offs"] = clean
        gm = res.get("gross_margin")
        if gm is not None:
            try:
                res["gross_margin"] = max(0.0, min(1.0, float(gm)))
            except Exception:
...
def _extract_financials(prompt: str, *, model: str, temperature: float) -> Dict[str, Any]:
    fin = _extract_financials_regex(prompt)
    # If sparse, try GPT for augmentation
    if not fin or any(k not in fin for k in ("starting_cash","monthly_burn","monthly_rev")):
        g = _extract_financials_gpt(prompt, model=model, temperature=temperature)
        fin = {**g, **fin} if g else fin
    # defaults
    fin.setdefault("starting_cash", 0.0)
    fin.setdefault("monthly_burn",  0.0)
    fin.setdefault("monthly_rev",   0.0)
    if fin.get("gross_margin") is not None:
        try:
            fin["gross_margin"] = float(fin["gross_margin"])
        except Exception:
            fin["gross_margin"] = None
    fin.setdefault("one_offs", [])
    return fin

def _compute_interval_finances(dt0: datetime, dt1: datetime, fin: Dict[str, Any]) -> Dict[str, float]:
    """Compute revenue, expenses, pnl over (dt0, dt1]."""
    elapsed_days = max(0.0, (dt1 - dt0).total_seconds() / 86400.0)
    if elapsed_days == 0:
        return {"revenue": 0.0, "expenses": 0.0, "pnl": 0.0}

    rev_per_day = (fin.get("monthly_rev", 0.0) / AVG_DAYS_PER_MONTH)
    opex_per_day = (fin.get("monthly_burn", 0.0) / AVG_DAYS_PER_MONTH)

    gm = fin.get("gross_margin")
    if gm is None:
        # If margin unknown, treat burn as total operating expenses and ignore COGS
        cogs_per_day = 0.0
    else:
        cogs_per_day = rev_per_day * (1.0 - max(0.0, min(1.0, gm)))

    revenue = rev_per_day * elapsed_days
    expenses = opex_per_day * elapsed_days + cogs_per_day * elapsed_days
    pnl = revenue - expenses
    return {"revenue": revenue, "expenses": expenses, "pnl": pnl}

def _apply_one_offs(prev_dt: datetime, cur_dt: datetime, fin: Dict[str, Any]) -> float:
    """Sum one-off cash adjustments occurring in (prev_dt, cur_dt]."""
    total = 0.0
    for ev in fin.get("one_offs", []):
        d: datetime = ev.get("date")
        amt: float = float(ev.get("amount") or 0.0)
        if isinstance(d, datetime) and prev_dt < d <= cur_dt:
            total += amt
    return total
...
def build_milestones(prompt: str, *, model: str = "gpt-4o-mini", temperature: float = 0.2, density: float = 0.5) -> Dict[str, Any]:
    ...
    if (not milestones) and _is_paragraph_only(prompt):
        ...
        milestones = infer_milestones_via_gpt(prompt, model=model, temperature=temperature, density=density)
    ...
    if milestones and augment_flag:
        ...
        extra = infer_milestones_via_gpt(prompt, model=model, temperature=temperature, density=density)
        if extra:
    ...
    refs = infer_references_via_gpt(prompt, model=model, temperature=temperature, density=density)
    if refs:
    ...
    # ---- NEW: Extract/Infer financials & compute cash timeline ----
    works.msg("💵 extracting financial parameters…")
    fin = _extract_financials(prompt, model=model, temperature=temperature)
    ...
    # Iterate milestones, accumulate cash
    cash = float(fin.get("starting_cash", 0.0))
    ...
    for idx, m in enumerate(milestones):
        ...
        # Finance for interval (prev_dt, dt]
        interval = _compute_interval_finances(prev_dt, dt, fin) if idx > 0 else {"revenue": 0.0, "expenses": 0.0, "pnl": 0.0}
        one_off = _apply_one_offs(prev_dt, dt, fin) if idx > 0 else 0.0

        cash_before = cash
        cash += interval["pnl"] + one_off
        cash_after = cash
        ...
        out_points.append({
            ...
            # --- NEW financials per milestone ---
            "cashStart": round(cash_before, 2),
            "revenueSincePrev": round(interval["revenue"], 2),
            "expensesSincePrev": round(interval["expenses"], 2),
            "oneOffsSincePrev": round(one_off, 2),
            "pnlSincePrev": round(interval["pnl"] + one_off, 2),
            "cashEnd": round(cash_after, 2),
        })
        prev_dt = dt
    ...
    return {
        ...
        "financials": {
            "starting_cash": fin.get("starting_cash", 0.0),
            "monthly_burn":  fin.get("monthly_burn", 0.0),
            "monthly_rev":   fin.get("monthly_rev", 0.0),
            "gross_margin":  fin.get("gross_margin"),
            "one_offs": [
                {"name": ev["name"], "date": ev["date"].isoformat(), "amount": ev["amount"]}
                for ev in fin.get("one_offs", [])
            ],
            "assumptions": "Revenue/expenses treated as continuous rates; pnl per interval = revenue - (opex + COGS if margin provided). One-offs applied when their date falls within an interval."
        }
    }
...
def _main_ion(default_model: str = "gpt-4o-mini") -> int:
    ...
    density = _clamp01(_read_param(4), 0.5)  # NEW
...
if __name__ == "__main__":
    works.msg("🔧 operations scheduler (milestones + optional refs + density + financials)")
    _main_ion()
