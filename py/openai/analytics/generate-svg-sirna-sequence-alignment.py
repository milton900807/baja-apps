...
RESIDUE_TOKEN_RE = re.compile(
    r"^(?:(?P<prefix_sugar>d|r))?(?P<base_mod>5m|m5)?(?P<base>[ACGTUacgtu])(?P<suffix_sugar>m|f|d|dna|rna|lna|moe|gna|cet|ome|omethyl|2f)?$",
    re.IGNORECASE,
)
TOKEN_RE = re.compile(
    r"\b(?:(?:d|r)?(?:5m|m5)?[ACGTUacgtu](?:m|f|d|dna|rna|lna|moe|gna|cet|ome|omethyl|2f)?)\b",
    re.IGNORECASE,
)
PLAIN_SEQ_RE = re.compile(r"\b[ACGTUacgtu]{6,}\b")
...
def _extract_sequence_blocks(description: str) -> List[List[str]]:
    ...
    for raw_line in description.splitlines():
        ...
        if len(plain) >= 2:
            return [list(plain[0]), list(plain[1])]
        if len(plain) == 1:
            blocks.append(list(plain[0]))
    if len(blocks) >= 2:
        return blocks[:2]
    plain_all = PLAIN_SEQ_RE.findall(description)
    if len(plain_all) >= 2:
        return [list(plain_all[0]), list(plain_all[1])]
    return []
...
def _tokens_to_strand(name: str, tokens: List[str], chemistry: str, source_chain_index: int) -> Strand:
    ...
    for i, token in enumerate(tokens, start=1):
        display_base, canonical_base, sugar = residue_token_to_parts(token, chemistry, preserve_explicit=True)
...
def _reverse_strand_preserving_mods(strand: Strand) -> Strand:
    residues = [Residue(index=i + 1, base=r.base, canonical_base=r.canonical_base, sugar=r.sugar, backbone_to_next="PO") for i, r in enumerate(reversed(strand.residues))]
    if residues:
        residues[-1].backbone_to_next = None
...
def parse_description(description: str) -> ParsedDescription:
    ...
    if len(sequence_blocks) < 2:
        raise ValueError("Expected two explicit sequence chains separated by whitespace or placed on separate lines.")
...
def _main() -> int:
    if works is not None:
        ...
        return 0
    import sys
    description = sys.argv[1] if len(sys.argv) > 1 else "UUUAAGAAGACAAAGGGUUUGG AAAGCCGACGAAAUUCUUCUGUUUCCCAAA"
