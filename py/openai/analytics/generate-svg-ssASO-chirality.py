...
ALLOWED_BACKBONES = {"PO", "PS", "PS2", "n001"}
ALLOWED_CHIRALITY = {"Rp", "Sp"}
...
class BackboneLink:
    ...
    def label(self) -> str:
        return f"{self.kind}({self.chirality})" if self.chirality else self.kind
...
class Residue:
    ...
    canonical_base: str
    sugar: str
    backbone_to_next: Optional[BackboneLink]
...
def canonical_backbone(token: str, default: str = "PO") -> Tuple[str, str]:
    ...
    if t == "n001":
        return ("n001", "N-(1,3-dimethylimidazolidin-2-ylidenyl) phosphoramidate")
    return ("PO", "phosphate")
...
def fuzzy_backbone(text: str) -> Optional[str]:
    t = text.lower().strip()
    if "n001" in t or "phosphoramidate" in t:
        return "n001"
...
def fuzzy_chirality(text: str) -> Optional[str]:
    t = text.strip()
    if re.search(r"\bSP\b", t, re.IGNORECASE):
        return "Sp"
    if re.search(r"\bRP\b", t, re.IGNORECASE):
        return "Rp"
    return None
...
def sugar_code_to_label(code: str) -> str:
    ...
    if code not in mapping:
        raise ValueError(f"Unknown sugar code: {code}")
    return mapping[code]


def canonical_base_from_token(base_token: str) -> str:
    b = base_token.upper()
    if b not in {"A", "C", "G", "U", "T"}:
        raise ValueError(f"Unknown base: {base_token}")
    return b


def linkage_from_symbol(symbol: str) -> BackboneLink:
    s = symbol.upper()
    if s == "S":
        return BackboneLink("PS", "Sp")
    if s == "R":
        return BackboneLink("n001", "Rp")
    if s == "O":
        return BackboneLink("PO", None)
    raise ValueError(f"Unknown linkage symbol in stereo pattern: {symbol}")


def parse_stereo_pattern(text: str) -> str:
    m = re.search(r"5\s*'\s*-?\s*([SROsro]+)\s*-?\s*3\s*'", text)
    if not m:
        raise ValueError("Could not find stereo pattern like 5'-SSRSS...-3'")
    return m.group(1).upper()


def tokenize_patent_stream(stream: str) -> List[str]:
    return [tok for tok in re.split(r"\s+", stream.strip()) if tok]


def parse_patent_construct(stereo_pattern: str, monomer_stream: str) -> List[Residue]:
    tokens = tokenize_patent_stream(monomer_stream)
    residues: List[Residue] = []
    residue_re = re.compile(r"^([fmrdl])([ACGTUacgtu])(\*)?$")

    for tok in tokens:
        tok = tok.strip()

        m = residue_re.match(tok)
        if m:
            sugar_code, base_code, star = m.groups()
            sugar = sugar_code_to_label(sugar_code)
            canonical_base = canonical_base_from_token(base_code)
            base_display = canonical_base
            inline_link = BackboneLink("PS", "Sp") if star else None
        ...
        if tok.lower() == "n001r":
            if not residues:
                raise ValueError("n001R cannot appear before the first residue")
            if residues[-1].backbone_to_next is not None:
                raise ValueError(
                    f"Residue {len(residues)} already has a linkage; cannot also apply n001R"
                )
            residues[-1].backbone_to_next = BackboneLink("n001", "Rp")
            continue

        raise ValueError(f"Unrecognized token in monomer stream: {tok}")

    expected_links = len(residues) - 1
    if len(stereo_pattern) != expected_links:
        raise ValueError(
            f"Stereo pattern has {len(stereo_pattern)} linkages, but monomer stream implies {expected_links}"
        )

    for i in range(expected_links):
        pattern_link = linkage_from_symbol(stereo_pattern[i])

        if residues[i].backbone_to_next is None:
            residues[i].backbone_to_next = pattern_link
        else:
            got = residues[i].backbone_to_next
            if got.kind != pattern_link.kind or got.chirality != pattern_link.chirality:
                raise ValueError(
                    f"Linkage mismatch at position {i+1}: "
                    f"stream says {got.label()}, stereo pattern says {pattern_link.label()}"
                )
    ...
    for i, base in enumerate(sequence, start=1):
        residues.append(
            Residue(
                ...
                sugar=sugar,
                backbone_to_next=BackboneLink("PO") if i < len(sequence) else None,
            )
        )
    return Strand(
        name="sense",
...
def apply_global_backbone(strand: Strand, backbone: str, chirality: Optional[str] = None) -> None:
    for i, r in enumerate(strand.residues):
        r.backbone_to_next = BackboneLink(backbone, chirality) if i < len(strand.residues) - 1 else None
...
def apply_backbone_positions(
    ...
    for pos in positions:
        if 1 <= pos < len(strand.residues):
            strand.residues[pos - 1].backbone_to_next = BackboneLink(backbone, chirality)
...
def parse_modifications(description: str, strand: Strand, chemistry: str) -> str:
    ...
    global_backbone_patterns = [
        r"\bfull\s+(PS2|PS|PO|n001|phosphorodithioate|phosphorothioate|phosphate|phosphodiester|phosphoramidate)(?:\((Rp|Sp)\))?\s+backbone\b",
        r"\bfull\s+(PS2|PS|PO|n001)(?:\((Rp|Sp)\))?\b",
    ]
    for pat in global_backbone_patterns:
        m = re.search(pat, text, re.IGNORECASE)
        if m:
            bb = fuzzy_backbone(m.group(1)) or m.group(1).upper()
            chirality = fuzzy_chirality(m.group(2) or "")
            if bb in ALLOWED_BACKBONES:
                apply_global_backbone(strand, bb, chirality)
                notes.append(f"full backbone={bb}{f'({chirality})' if chirality else ''}")
                break
    ...
    for m in re.finditer(
        r"\b(PS2|PS|PO|n001|phosphorodithioate|phosphorothioate|phosphate|phosphodiester|phosphoramidate)(?:\((Rp|Sp)\))?\s+(?:at|on)\s+([0-9,\-\s;]+)\b",
        text,
        re.IGNORECASE,
    ):
        bb = fuzzy_backbone(m.group(1)) or m.group(1).upper()
        chirality = fuzzy_chirality(m.group(2) or "")
        positions = parse_positions(m.group(3))
        if bb in ALLOWED_BACKBONES and positions:
            apply_backbone_positions(strand, positions, bb, chirality)
            notes.append(
                f"{bb}{f'({chirality})' if chirality else ''} at link positions {positions}"
            )
...
def style_from_strand(strand: Strand, chemistry: str) -> StrandStyle:
    ...
    first_link = next((r.backbone_to_next for r in strand.residues if r.backbone_to_next), BackboneLink("PO"))
    backbone_label, backbone_note = canonical_backbone(first_link.kind, "PO")
    if first_link.chirality:
        backbone_label = f"{backbone_label}({first_link.chirality})"
        backbone_note = f"{backbone_note} ({first_link.chirality})"
...
def parse_description(description: str) -> ParsedDescription:
    ...
    patent_like = ("n001R" in text) or bool(re.search(r"\b[fmrdl][ACGTU]\*?\b", text, re.IGNORECASE))
    stereo_like = bool(re.search(r"5\s*'\s*-?\s*[SROsro]+\s*-?\s*3\s*'", text))

    if patent_like and stereo_like:
        stereo = parse_stereo_pattern(text)
        monomer_stream = re.sub(r"5\s*'\s*-?\s*[SROsro]+\s*-?\s*3\s*'\.?", "", text).strip()
        residues = parse_patent_construct(stereo, monomer_stream)
        ...
        style = style_from_strand(strand, "RNA")

        return ParsedDescription(
            strand_type="single",
            chemistry="RNA",
            sense_seq="".join(r.canonical_base for r in residues),
            antisense_seq=None,
            title="Single-stranded oligonucleotide schematic",
            sense_style=style,
            antisense_style=StrandStyle("", "", "", ""),
            interpretation_source="patent_style_sequence_parser",
            chemistry_recipe=f"stereo pattern={stereo}; mixed sugar/base/linkage stream parsed directly",
            seed_sequence="".join(r.canonical_base for r in residues),
            sequence_source="explicit patent-style notation",
            chemistry_source="direct token parser",
            strands=[strand],
        )
...
class SvgBuilder:
    ...
    def circle(
        ...
        self.add(
            ...
            f'stroke="{stroke}" stroke-width="2"{extra_attrs} />'
        )
...
def draw_backbone_segment(
    ...
    # connector line on strand axis
    builder.line(left, y1, right, y2, stroke_width=2.4, stroke="#D1D5DB")

    # move backbone label ABOVE the strand
    builder.text(
        mid,
        y1 - 34,
        link.label(),
...
def draw_single_stranded(parsed: ParsedDescription) -> str:
    ...
    # lower the strand a bit to create headroom for backbone labels above it
    y = 215
    start_x = 110
    width = max(860, 220 + (n * step))
    height = 360

    # push monomer/base labels farther up so they don't clash with backbone labels
    base_offset = 88
    ...
    for i in range(n - 1):
        x1 = start_x + i * step
        x2 = start_x + (i + 1) * step
        link = strand.residues[i].backbone_to_next or BackboneLink("PO")
        draw_backbone_segment(svg, x1, y, x2, y, sugar_r=sugar_r, link=link)

    for i, residue in enumerate(strand.residues):
        x = start_x + i * step
        draw_residue(svg, residue, x, y, sugar_r=sugar_r, base_offset=base_offset)
...
def _main() -> int:
    ...
    description = (
        sys.argv[1]
        if len(sys.argv) > 1
        else "5'-SSRSSRSSOSSSOSSSRSS-3'. "
             "fC* fU* fC n001R fC* fG* fG n001R fU* fU* mC fU* mG* fA* mA "
             "fG* fG* fU* fG n001R fU* fU* fC"
    )
