...
def image_to_svg_output(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Preprocess tuned for black/white line drawings and molecular stick diagrams.
    ...
    clahe_clip_limit = float(params.get("clahe_clip_limit", 1.6))
    clahe_tile_grid_size = int(params.get("clahe_tile_grid_size", 8))
    median_blur = int(params.get("median_blur", 3))
    ...
    adaptive_block_size = int(params.get("adaptive_block_size", 31))
    adaptive_c = int(params.get("adaptive_c", 12))
    open_iterations = int(params.get("open_iterations", 0))
    close_iterations = int(params.get("close_iterations", 1))
    min_component_area = int(params.get("min_component_area", 6))

    # Edge extraction.
    low_thresholds = params.get("low_thresholds", [24, 40, 64])
    high_thresholds = params.get("high_thresholds", [72, 120, 180])
    dilate_iterations = int(params.get("dilate_iterations", 0))

    # Geometry shaping.
    polygon_epsilon = float(params.get("polygon_epsilon", 2.0))
    line_epsilon = float(params.get("line_epsilon", 1.1))
    min_polygon_area = float(params.get("min_polygon_area", 50.0))
    min_line_length = float(params.get("min_line_length", 12.0))
