    ...
    *,
    ...
    try:
        ...
        user_msg = (
            ...
            "- NO DUPLICATE ROWS.\n"
            "- All cells in the first column will have a globally unique label that describes the unique formula in the second column.\n"
...
# ---------- Units enforcement ----------
def __removeMe(final_json: dict, default_unit: str = "unitless") -> dict:
    data = dict(final_json or {})
    tables = dict(data.get("tables") or {})
...
def remove_star_after_digit(s: str) -> str:
    # Turn "Year_1*_Revenue" into "Year_1_Revenue" (only inside labels)
...
def enforce_units(final_json: dict, default_unit: str = "unitless") -> dict:
    ...
    # Force units to be a dict (ignore bad shapes like str/list/None)
    ...
    for t, label_set in labels_by_table.items():
        # Ensure per-table map is a dict
        if not isinstance(units.get(t), dict):
            units[t] = {}
...
def run_two_stage(
    ...
    *,
    ...
    previous_results: Optional[dict] = None,
    max_first_pass_retries: int = 1,   # <-- configurable retries
) -> Dict[str, Any]:
    """
