        ...
        class Interpreter {
            ...
            async executeCommand(line) {
                ...
                if (this.testScopeFormat(line.trim())) {
                    ...
                    if (command.includes(".")) {
                        const [objectName, method] = command.split(".");
                        this.currentObject = this.objects[objectName];
                        ...
                        throw new Error(`Unknown command: ${command.toLowerCase()}`);
                    }
                }
            }
                ...
                if (values[0] === 'on') {
                    ...
                    if (values[1] === 'on') {
                        ...
                        if (!this.ref.selectWellsByString) {
                            pt.setMessage("Scope is incorect.")
                            return;
                        }
                        this.ref.selectWellsByString(values[0])
                        let from_selected_wells = this.ref.getSelectedWells();
                        let category = from_selected_wells[parseInt(values[2])]
                        for (let i of category) {
                            if (i.value && typeof i.value === 'string') {
                                i.value = i.value.trim();
                            }
                        }
                        let agv_sum = this.calculateAggregatesByValue(from_selected_wells, parseInt(values[2]))
                        let agv = agv_sum.aggregates
                        if (agv_sum.count === 1) {
                            pt.setMessage(agv_sum.count + " group found.")
                        }
