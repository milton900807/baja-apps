function (lib_id, file_id) {
    ...
    showWidget({
        ...
        exec('flexigraph/gene.js', progressBar).then(async (graph) => {
            ...
            window.history.pushState({ libid: lib_id }, 'Screen', `app/baja/screens/editor-eln.js?lib_id=${lib_id}&file_id=${file_id}`);
            if (file['folder']) {
                folder = file;
            }
            ...
            track_items.push({
                'label': 'Paste...', 'ionfunction': createIonFunction(async () => {
                    ...
                    CurrentLayout.clearComponent('buttonMenuPanel|labelPanel')
                    CurrentLayout.setComponent('buttonMenuPanel', v);
            ...
            tools_menu = [
                ...
                {
                    'label': 'Registration DB',
                    'ionfunction': createIonFunction(async () => {
                        exec('baja/screens/menu/compound-registration.js', graph, genegraph_panel_layout, lib_id);
...

                {
                    'label': 'Off-targets', 'ionfunction': createIonFunction(async () => {
                        ...
                        CurrentLayout.clearComponent('buttonMenuPanel|labelPanel')
                        await exec('baja/screens/menu/off-target-tools-sub-menu.js', graph, genegraph_panel_layout)
                    })
                },
                {
                    'label': 'Sequence', 'ionfunction': createIonFunction(async () => {
                        let hl = await exec('baja/screens/menu/track-sequence.js', graph, genegraph_panel_layout)
                        CurrentLayout.clearComponent('buttonMenuPanel|labelPanel')
                ...
                {
                    'label': 'Synthesis', 'ionfunction': createIonFunction(async () => {

                        await exec('baja/screens/menu/synthesis-tools.js', graph, genegraph_panel_layout, lib_id)
            ...
            let file_items = [
                ...
                {
                    'label': 'Paste (OCR)', 'ionfunction': createIonFunction(async () => {
                        try {
                                            ...
                                            },
                                            {
                                                ...
                                                'component':
                                                {
                                                    ...
                                                    data: {
                                                        ...
                                                        showButton: false,
                                                        listItems: [IMAGE_OCR, ENSEMBL_IDS, TEXT],
            ...
            file_items.push({
                'label': 'Share', 'ionfunction': createIonFunction(async () => {
                    let host_ = window['env']['appHost']
                    let tp = path;
                    if (path.startsWith('/myfiles/')) {
                        tp = path.substring(9)
                    }

                    infoPrompt("Share link: " + `${host_}/app/screen/editor?path=/${getUser()}/${tp}`)
