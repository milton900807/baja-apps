function (ruleBlk, graph, logPanel) {
    return new Promise(async (resolve, reject) => {
        ...
        let ruleSeedSeq = await exec('baja/screens/annotation/rule-filter-seed-seq.js');
        let phaseRule = await exec('baja/screens/annotation/phase-rule.js');
        let spaceRule = await exec('baja/screens/annotation/space-rule.js');
        let ot = await exec('baja/screens/annotation/offtarget-rule.js');
        let crossRx = await exec('baja/screens/annotation/cross-rx-rule.js');
        let randomRule = await exec('baja/screens/annotation/random-rule.js');
        let palindromeRule = await exec('baja/screens/annotation/palindrome-rule.js');
        let termoRule = await exec ('baja/screens/annotation/thermo-rule.js')
                                ...
                                for (let t of graph.track) {
                                    if (t.oligos.length > 0) {
                                        ...
                                        if (t.targetPhase) {
                                            ...
                                            let [splicedtrack, splicedindices] = await exec('baja/screens/annotation/splice-indels.js', t, phaseselect);
