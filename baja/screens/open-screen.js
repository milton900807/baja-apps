function (lib_id, file_id) {
    ...
    showWidget({
        wid: 'html',
        data: '<hr> Loading bajabio screening editor... '
    }).then(async working => {
        ...
        exec('flexigraph/gene.js', progressBar).then(async (graph) => {
            ...
            if (file && file.name) {
                ...
                graph.parentId = file['parentReference']['id']

            }
            let Icon = await exec('flexigraph/shapes/icon.js')
            window.history.pushState({ libid: lib_id }, 'Screen', `app/baja/screens/open-screen.js?lib_id=${lib_id}&file_id=${file_id}`);
            if (file['folder']) {
                folder = file;
            }
            ...
            if (path && path.endsWith('.screen')) {
                let host_ = window['env']['apiUrl']
                let index = path.lastIndexOf('/')
                if ((user != null) || path.startsWith('/myfiles/')) {
                    let jsonobj = {
                        'path': path,
                        'key': 'user',
                        'user': getUser()
                        ...
                        if (imgs[i].type.indexOf("text/plain") >= 0) {
                            imgs[i].getAsString(async (s) => {
                                ...
                                if (tableObjects && tableObjects.length > 0) {
                                    ...
                                    console.log(" JS " + (pat).toString())
                                    graph.setMessage("NCBI->ENSEMBL annotations... ")
            ...
            track_items.push({
                'label': 'Track DB', 'ionfunction': createIonFunction(async () => {

                    let script_canvas = await exec('baja/screens/menu/trackdb.js', graph, genegraph_panel_layout)
                    CurrentLayout.clearComponent('buttonMenuPanel|labelPanel')
                    CurrentLayout.setComponent('buttonMenuPanel', script_canvas);
            ...
            track_items.push({
                'label': 'Paste...', 'ionfunction': createIonFunction(async () => {
                    let v = await exec('screen/controls/paste-panel.js', graph, genegraph_panel_layout, eeditor_state)
            ...
            let showSaveScreen = async () => {
                console.log('debubg');
                let t2 = await exec('baja/screens/my-saved-screens-w.js', lib_id, folder.id, graph, 'save', genegraph_panel_layout)
                let save_tab = {
                    wid: 'card',
                    data: {
            ...
            let saveSaveScreen = async () => {
                let savedScreens = await exec('screen/io/save-obj.js', graph, genegraph_panel_layout)
                showModal(savedScreens);
            }
            ...
            tools_menu = [
...

                {
                    'label': 'Annotations', 'ionfunction': createIonFunction(async () => {
                        ...
                        let select_panel = await exec('baja/screens/menu/annotation/annotation-tools2.js', graph, genegraph_panel_layout)
            ...
            let file_items = [
                ...
                {
                    'label': 'Paste (text)', 'ionfunction': createIonFunction(async () => {
                        exec('baja/util/paste-panel.js', '', graph, genegraph_panel_layout, eeditor_state)
                ...
                },
                {
                    'label': 'Clear Graph Layers', 'ionfunction': createIonFunction(async () => {
                        ...
                        let confirm = await exec('baja/lib/confirm.js', 'Are you sure you want to delete graph layers?', async () => {
                            graph.layers = []
                                    ...
                                    [
                                        {
                                            ...
                                            {
                                                wid: 'html',
                                                data: '<font color=red> Download xlsx  </font>'
                                            }
                                        },
                                        {
                                            'title': '',
            ...
            if (user != null && user.length > 0) {
                ...
                file_items.push({
                    'label': 'Open from scratch folder', 'ionfunction': createIonFunction(() => {
                        openSaveScreen()
                                                ...
                                                'label': 'Select', 'items': [
...

                                                    {
                                                        'label': 'Annotations', 'ionfunction': createIonFunction(async () => {
                                                            ...
                                                            let hl = await exec('baja/screens/menu/annotation-tools-simple.js', graph, genegraph_panel_layout)
                                                            CurrentLayout.clearComponent('buttonMenuPanel|labelPanel')
                                                            CurrentLayout.setComponent('buttonMenuPanel', hl);
                                                            CurrentLayout.setComponent('labelPanel', select_panel);
