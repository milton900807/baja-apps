                                    ...
                                    buttons: [
...

                                        {
                                            label: 'Run..', ionFunction: createIonFunction(async () => {
                                                ...
                                                let polygons = selectedTrackLayer.polygonpts;
                                                let vinter = [];
                                                let increment = window.getWidgetValue();
                                                increment = parseInt(increment);

                                                for (let i = start; i < end; i += increment) {
                                                    let overlappingIntervals = polygons.filter(interval =>
                                                        interval.x < i + increment && interval.x > i
                                                    );

                                                    for (let polygon of overlappingIntervals) {
                                                        let center = polygon.x;
                                                        ...
                                                        if (seqRange.length === increment) {
                                                            let entry = {
                                                                ...
                                                                g_interval: { gxi: windowStart, gxf: windowEnd },
                                                                seq: seqRange,
                                                                seq_len: seqRange.length,
                                                                x: polygon.x,
                                                                y: polygon.y
                                                            };
                                                            vinter.push(entry);
                                                ...
                                                let r = await exec('py/tracks/find-seq-features-for-y.py', vinter)
                                                let sw = 300;

                                                const grid = new MGrid(gx, gy, sw, sw);
                                                let cpt = new TrackLayer('seq-features', selectedTrack.tgraph.xmin, selectedTrack.tgraph.ymin, selectedTrack.tgraph.xmax, selectedTrack.tgraph.ymax);
                                                let polygonPoints = [];
                                                const featureImportances = r.feature_importances;
                                                const featureImportanceValues = r.feature_importance_values;
                                                featureImportances.forEach((importance, index) => {
                                                    let x = importance;
                                                    let y = featureImportanceValues[x];
                                                    polygonPoints.push({ x: selectedTrack.xi + x, y: y });
                                                });

                                                for (let p of polygonPoints) {
                                                    let center = p.x;
                                                    let xii = center - (increment / 2);
                                                    let xff = center + (increment / 2);
                                                    const yvalue = (p.y * 2);
                                                    cpt.addInterval(xii, xff, yvalue, '' + yvalue)
                                                }

                                                let pcaplot = new PCAPlot(r, grid);
                                                pcaplot.name = selectedTrack.name + ': ' + selectedTrack.description
                                                pcaplot.layers.push(cpt)
                                                if (gx && gy) {
                                                    pcaplot.x = gx;
                                                    pcaplot.y = gy;
                                                }
                                                pcaplot.scatterData.points = polygonPoints;
                                                graph.plots.push(pcaplot)

                                                selectedTrack.addLayer(cpt)
                                                if (!graph.plots) {
                                                    graph.plots = []
                                                }
