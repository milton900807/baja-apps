        ...
        if (oligo.type === 'amplicon') {
            ...
            const models = [
...

                {
                    ...
                    click: async (xwc, ywc) => {
                        ...
                        graph.rungraph(async () => {
                            await exec('baja/screens/menu/probe-action.js', graph, genegraph_panel_layout, attr_window);
                        });
...

                {
                    ...
                    click: async (xwc, ywc) => {
                        ...
                        if (l && r && m) {
                            await exec('baja/screens/menu/run-off-targets.js', graph, genegraph_panel_layout, [l, r, m]);
                        } else if (l && r) {
                            await exec('baja/screens/menu/run-off-targets.js', graph, genegraph_panel_layout, [l, r]);
                        }
                    },
                    move: () => {
                        log('');
...

                {
                    ...
                    click: async (xwc, ywc) => {
                        ...
                        if (l) {
                            ...
                            l.offtarget = null;
                            await exec('baja/screens/menu/run-off-targets.js', graph, genegraph_panel_layout, [l]);
                        }
                    },
                    move: () => {
                        log('');
                    ...
                    click: async (xwc, ywc) => {
                        ...
                        if (oligo.type === 'amplicon') {
                            ...
                            if (left && left.offtarget) {
                                let rs = await exec('https://data.lajollalabs.com/ionworks/py/gene/gff.py', JSON.stringify(left.offtarget));
                                let component = await exec('baja/screens/menu/menu-for-single-aso-display-report.js', graph, genegraph_panel_layout, rs, " Left:  " + left.synthesisSequence);
                                cards.push(component);
                            }

                            if (right && right.offtarget) {
                                let rs = await exec('https://data.lajollalabs.com/ionworks/py/gene/gff.py', JSON.stringify(right.offtarget));
                                let component = await exec('baja/screens/menu/menu-for-single-aso-display-report.js', graph, genegraph_panel_layout, rs, " Right:  " + right.synthesisSequence);
                                cards.push(component);
                            }

                            if (mid && mid.offtarget) {
                                let rs = await exec('https://data.lajollalabs.com/ionworks/py/gene/gff.py', JSON.stringify(mid.offtarget));
                                let component = await exec('baja/screens/menu/menu-for-single-aso-display-report.js', graph, genegraph_panel_layout, rs, " Probe:  " + mid.synthesisSequence);
                                cards.push(component);
                            }
                                            ...
                                            buttons: [
                                                {
                                                    ...
                                                    ionFunction: createIonFunction(async () => {
                                                        let button_canvas = await exec('screen/controls/navigation-panel.js', graph, graph.genegraph_panel_layout);
                            ...
                            let rs = await exec('https://data.lajollalabs.com/ionworks/py/gene/gff.py', oligo);
                            let component = await exec('baja/screen/menu/menu-for-single-display-report.js', graph, genegraph_panel_layout, rs);
                            let cards = [component];
            ...
            const models = [
...

                {
                    ...
                    click: async (xwc, ywc) => {
                        await exec('baja/screens/menu/run-off-targets.js', graph, genegraph_panel_layout, [oligo]);
                    },
                    move: () => {
                        log('');
                    }
...

                {
                    ...
                    click: async (xwc, ywc) => {
                        ...
                        let rs = await exec('https://data.lajollalabs.com/ionworks/py/gene/gff.py', JSON.stringify(oligo.offtarget));
