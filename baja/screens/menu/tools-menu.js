    ...
    tools_menu = [
        {
            'label': 'Assay design:  Design primers etc.', click: (async () => {
                ...
                setTimeout(async () => { await exec('baja/screens/menu/assay-tools.js', graph, genegraph_panel_layout) }, 1000)
        ...
        },
        {
            'label': 'Splicing models', click: (async () => {
                CurrentLayout.clearComponent('buttonMenuPanel,labelPanel')
                setTimeout(async () => {
                    let hl = await exec('baja/screens/menu/splicing/splicing-tools2.js', graph, genegraph_panel_layout)
                    CurrentLayout.clearComponent('buttonMenuPanel|labelPanel')
...

        {
            'label': 'Compare sequences, tracks annotations etc...', click: (async () => {
                ...
                let hl = await exec('baja/screens/menu/comparative-tools.js', graph, genegraph_panel_layout)
                CurrentLayout.clearComponent('buttonMenuPanel|labelPanel')
                CurrentLayout.setComponent('buttonMenuPanel', hl);
                CurrentLayout.clearComponent('mainPanel')
                CurrentLayout.setComponent('mainPanel', genegraph_panel_layout);
        ...
        },
        {
            'label': 'Filter', click: (async () => {
                CurrentLayout.clearComponent('buttonMenuPanel|labelPanel')
                await exec('baja/screens/menu/filter-sub-menu.js', graph, genegraph_panel_layout)
                CurrentLayout.clearComponent('mainPanel')
                CurrentLayout.setComponent('mainPanel', genegraph_panel_layout);
        ...
        },
        {
            'label': 'Plots', click: (async () => {
                let hl = await exec('baja/screens/menu/plot-editor.js', graph, genegraph_panel_layout)
                CurrentLayout.clearComponent('buttonMenuPanel|labelPanel')
                CurrentLayout.setComponent('buttonMenuPanel', hl);
            })
        },
