    ...
    exec('flexigraph/gene.js').then(async (graph) => {
        ...
        let showSavedScreens = async () => {
            ...
            if (!readonly) {
                ...
                let mylibs = await exec('baja/screens/my-libraries.js', graph)
                showModal(mylibs);
        ...
        track_items.push({
            'label': 'Layers...', 'ionfunction': createIonFunction(async () => {
                exec('baja/screens/menu/track-layer-editor.js', graph)
            })
        })
        track_items.push({
            'label': 'Export...', 'ionfunction': createIonFunction(async () => {
                exec('baja/screens/menu/export-track-details.js', graph)
            })
        })
        ...
        let plate_panel = {
            ...
            data: createIonFunction(async () => {
                ...
                if (graph && graph.track && graph.track.length > 0) {
                    ...
                    let order_panel = await exec('baja/plate/plate-order.js', total_oligos, graph)
                    if (total_oligos.length > 0) {
                        return {
                            "style.padding-top": '10px',
                            cards: [
                                [
        ...
        let showSaveScreen = async () => {
            ...
            if (readonly)
                t2 = await exec('baja/screens/my-libraries', graph, 'save', genegraph_panel_layout)
                    ...
                    tools_menu = [
...

                        {
                            'label': 'Variants', 'ionfunction': createIonFunction(async () => {
                                ...
                                let hl = await exec('baja/screens/menu/variant-tools.js', graph)
                                CurrentLayout.clearComponent('buttonMenuPanel|labelPanel')
                                CurrentLayout.setComponent('buttonMenuPanel', hl);
                            })
                        },
                                        ...
                                        'buttons': [
                                            {
                                                x: 0, y: 0, label: 'Selected Compounds', ionFunction: createIonFunction(async () => {
                                                    ...
                                                    await exec('baja/screens/menu/select-compound-for-offtarget.js', library.id, graph, showMainScreen, genomes)
                                                })
                                            },
                                            {
        ...
        let synthesis_menu = [
            ...
            },
            {
                ...
                'label': 'Export Off-target report', 'ionfunction': createIonFunction(async () => {
                    let trackName = '';
                    for (let t of graph.track) {
                        trackName += t.name + '__';
                        let oligos = t.oligos;
                        ...
                        if (oligos.offtarget) {
                            for (let data of oligos.offtarget) {
                                const csvContent = [];
        ...
        let ____main_menu_bar_deprecated_for_minimal = {
            'width': '100%',
            'component': {
                wid: 'menu',
                data: {
                    menus: [
                            ...
                            'label': 'File', 'items': [
                                ...
                                },
                                {
                                    ...
                                    'label': 'Bookmarks to slideshow', 'ionfunction': createIonFunction(async () => {
                                        ...
                                        let m = await exec('baja/screens/modal/slide-show-path-text.js', graph, genegraph_panel_layout);
                                        CurrentLayout.clearComponent('mainPanel')
                        ...
                        },
                        {
                            'label': 'Select', 'items': [
                                ...
                                await exec('baja/screens/menu/select-tracks.js', library.id, graph, button_canvas),
                            ...
                            'label': 'Navigate', 'items': [
                                ...
                                },
                                {
                                    'label': 'Go to track coordinate', 'ionfunction': createIonFunction(async () => {
                                        ...
                                        await exec('baja/screens/menu/navigate-track.js', graph)
                                    })
                                },
                                {
