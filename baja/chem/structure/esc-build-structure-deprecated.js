function (chemObj, bioObj) {
    ...
    return new Promise(async (res, rej) => {
        ...
        let antisense =    "m(?)[sp].[fl2r](?)[sp].m(?)p.m(?)p.m(?)p.[fl2r](?)p.[fl2r](?)p.[fl2r](?)p.[fl2r](?)p.m(?)p.m(?)p.m(?)p.[fl2r](?)p.m(?)p.[fl2r](?)p.m(?)p.m(?)p.m(?)p.m(?)p.m(?)p.m(?)[sp].m(?)[sp].m(?)";
        ...
        if ( strand < 0 ) {
            if ( senseSeq.length > 21 ){
                senseSeq = targetSequence.substring ( 0, 21 )
            }
            synthesisSeq = Biopolymer.comp(senseSeq)
            senseStructure = Biopolymer.applySequenceToTemplate(sense, Biopolymer.reverse(senseSeq));
            antisenseStructure = Biopolymer.applySequenceToTemplate(antisense, (synthesisSeq));
