function (graph, folder, reference_object) {
                                    ...
                                    data: {
                                        ...
                                        "ionfunction.fileClick": createIonFunction(async (element) => {
                                            ...
                                            setTimeout(async () => {
                                                ...
                                                if (path.endsWith('.bjb-share')) {
                                                    let rs = await POSTJSON(jsonobj, host_ + '/load-file');
                                                    jsonobj.path = rs.path;
                                                }

                                                let rs = await POSTJSON(jsonobj, host_ + '/load-file');

                                                console.log(" rs " + rs.plateTrack)
                                                const bs = (rs.plateTrack)
                                                let gs = JSON.stringify(bs, function (key, value) {
                                                ...
                                                folder.wells[0][0].properties['package'] = stringData;
