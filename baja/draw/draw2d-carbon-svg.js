    ...
    try {
        ...
        let hd = {
            ...
            mouseDownListener: async (x, y) => {
                ...
                // first click must land on an existing node / endpoint
                if (clickIndex === 1) {
                    ...
                    const preview = snappedEndpointFromPointer(
                        grid,
                        hd.startWx,
                        hd.startWy,
                        wx,
                ...
                if (clickIndex === 2) {
                    const snapped = snappedEndpointFromPointer(
                        grid,
                        hd.startWx,
                        hd.startWy,
                        wx,
        ...
        function collectLineEndpoints(platetrack) {
            ...
            const visitShape = (shape) => {
                ...
                // some glyph containers may wrap shape under .shape
                if (shape.shape && shape.shape !== shape) {
                    visitShape(shape.shape);
                }
            };

            // try a few likely canvas collections
            const pools = [
                platetrack?.glyphs,
                platetrack?.drawables,
                platetrack?.items,
                platetrack?.wbv?.glyphs,
                platetrack?.model?.glyphs
            ];
