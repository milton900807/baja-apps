    ...
    return new Promise(async (resolve, reject) => {
        ...
        let Plate = class Plate {
                ...
                this.bottom_buttons = [
                    {
                        name: "-", x: 0 + bsize, y: 10, width: 20, height: 20, action: async (bx, by, x, y, pt) => {
                            let confirm = await exec('baja/lib/confirm.js', 'Delete the last row? (ctrl+z to undo)', async () => {
                                // removeRowsDown pushes the one undo snapshot for this deletion
            ...
            drawButtons(ctx, graph, __sw) {
                ...
                // DRAWING AND HIT-TESTING OFF THE SAME SET. inButtons reads button_set, which
                // drawSimpleButtons reassigns and never puts back -- a table that once drew a
                // simple set then drew this one kept hit-testing the other, and the two sets
            ...
            // NO WAIT ON THE BUTTONS ANY MORE. They used to be deaf for a second after they
            // appeared, because the RELEASE fired them: the click that selected a table, or
            // the release that ended a drag by its bar, landed on a button that had just been
            // painted under the pointer. Buttons answer the PRESS now (fireBarButton), and
            // during the press that selects a window its buttons are not drawn yet -- so the
            // thing the wait was guarding against cannot happen, and a button that is on
            // screen answers the moment it is pressed. The CELL buttons keep their own wait
            // (__cellBtnReady): those appear under the pointer while it is already down.
            ...
            clk_drag(pt) {
                ...
                const getCellButtonRects = () => {
                    ...
                    const gap = 10;
                    const buttonY = this.__cellBtnY(pt.selected_well, 20);

                    return this.txbuttons.map((button, index) => {
                        const buttonHeight = button.height || buttonWidth;
                        const buttonX = this.__cellBtnX(pt.selected_well, index, buttonWidth);
            ...
            drawTextBoxWithCursor(ctx, text, cursorPos, x, y, width, height, isSelected, style, selected_well) {
                ...
                try {
                    ...
                    if (
                        ...
                        const buttonY = this.__cellBtnY(selected_well, 20);

                        this.txbuttons.forEach((button, index) => {
                            const buttonX = this.__cellBtnX(selected_well, index, buttonWidth);
                            const buttonHeight = button.height || buttonWidth;
                            const circleRadius = Math.min(buttonWidth, buttonHeight) / 2;
                            this._paintTableButton(ctx, button, buttonX + buttonWidth / 2, buttonY + buttonHeight / 2,
                                circleRadius, button === this.highlightbutton, 'cell');
                        });
                    }
                } finally {
                    ctx.restore();
