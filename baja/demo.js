function (price) {
    ...
    showWidget({
        ...
        exec('flexigraph/gene.js', progressBar).then(async (graph) => {
            ...
            tools_menu = [
                {
                    'label': 'AI Models', 'ionfunction': createIonFunction(async () => {
                        graph.showWindowMenu(await exec('baja/demo-models', graph, genegraph_panel_layout), 10, 10, 400)
                    })
                },
                {
                    'label': 'Assay design', 'ionfunction': createIonFunction(async () => {
                        CurrentLayout.clearComponent('buttonMenuPanel|labelPanel')
                        graph.setMessage(" License required. ")
            ...
            file_items.push(
                {
                    'label': 'New track', 'ionfunction': createIonFunction(() => {
                        exec('baja/manchester/add-track.js', graph)
                    })
                },
                {
            ...
            file_items.push({
                'label': 'Copy graph', 'ionfunction': createIonFunction(() => {
                    graph.setMessage(" Copy graph... ")
                    let v = graph.getState();
            ...
            setTimeout(() => {
                ...
                graph.setMessageCenter(' [Track] -> [Add] to add a transcript', 40)
