function (lib_id, file_id) {
    ...
    showWidget({
        ...
        exec('flexigraph/gene.js', progressBar).then(async (graph) => {
            ...
            window.history.pushState({ libid: lib_id }, 'Screen', `app/baja/manchester/open-screen.js?lib_id=${lib_id}&file_id=${file_id}`);
            if (file['folder']) {
                folder = file;
            }
            ...
            track_items.push({
                'label': 'Track DB', 'ionfunction': createIonFunction(async () => {
                    ...
                    let script_canvas = await exec('baja/manchester/menu/trackdb.js', graph, genegraph_panel_layout)
                    CurrentLayout.clearComponent('buttonMenuPanel|labelPanel')
                    CurrentLayout.setComponent('buttonMenuPanel', script_canvas);
            ...
            track_items.push({
                'label': 'Paste...', 'ionfunction': createIonFunction(async () => {
                    let v = await exec('manchester/controls/paste-panel.js', graph, genegraph_panel_layout, eeditor_state)
            ...
            let showSaveScreen = async () => {
                console.log('debubg');
                let t2 = await exec('baja/manchester/my-saved-screens-w.js', lib_id, folder.id, graph, 'save', genegraph_panel_layout)
                let save_tab = {
                    wid: 'card',
                    data: {
