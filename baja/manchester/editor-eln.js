function (lib_id, file_id) {
    ...
    showWidget({
        ...
        exec('flexigraph/gene.js', progressBar).then(async (graph) => {
            ...
            window.history.pushState({ libid: lib_id }, 'Screen', `app/baja/manchester/editor-eln.js?lib_id=${lib_id}&file_id=${file_id}`);
            if (file['folder']) {
                folder = file;
            }
            ...
            tools_menu = [
                ...
                },
                {
                    ...
                    'ionfunction': createIonFunction(async () => {
                        exec('baja/manchester/menu/compound-registration.js', graph, genegraph_panel_layout, lib_id);
                ...
                {
                    'label': 'Sequence', 'ionfunction': createIonFunction(async () => {
                        let hl = await exec('baja/manchester/menu/track-sequence.js', graph, genegraph_panel_layout)
                        CurrentLayout.clearComponent('buttonMenuPanel|labelPanel')
            ...
            file_items.push({
                'label': 'Share', 'ionfunction': createIonFunction(async () => {
                    ...
                    infoPrompt("Share link: " + `${host_}/app/manchester/editor?path=/${getUser()}/${tp}`)
