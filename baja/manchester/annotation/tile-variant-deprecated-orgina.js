function (variant, selectedTrack, graph, opposite, all) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        let rchemistry_template = `([?]moe.sp.){1}([?]moe.p.) {4}([?]d.sp){10}([?]moe.p){2}([?]moe.sp){2}([?]moe){1}`;
        let base_count = Biopolymer.countBasesOligoScript(rchemistry_template);
        ...
        if ( plot ) {
            ...
            for (let i = 1; i < splicedtrack.length - base_count; i++) {
                ...
                if ( !(existingOligos.includes(sequence)) ) {
                    let anno = new Oligo('gapmer', sequence, '', start, end, yy);
                    let ytmp = 0.15;
                    ...
                    if ( selectedTrack.strand > 0 ) {
                        ...
                        anno.synthesisSequence = anno.sequence.split("").reverse().join("").replace(/[A,C,T,G,N]/gi, m => ({
                            'C': 'G',
                            'G': 'C',
                            'A': 'T',
                            'T': 'A',
                        ...
                        anno.synthesisSequence = anno.sequence.replace(/[A,C,T,G,N]/gi, m => ({
                            'C': 'G',
                            'G': 'C',
                            'A': 'T',
                            'T': 'A',
                    ...
                    anno.structure = Biopolymer.createOligoStr(rchemistry_template, anno.synthesisSequence);
