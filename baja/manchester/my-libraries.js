function (graph, type, genegraph_panel_layout) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        let duser = await client.api(`/drives`).get();
        let icon = await exec('baja/images/lib2.js');
        for (let u of duser['value']) {
            if (u.name === 'My Data') {
                library = u;
            }
        }
            ...
            resolve(
                {
                    ...
                    data: {
                        path: `/drives/${libraryid}/items/${folderid}`, 'ionfunction.path': createIonFunction(async (file) => {
                            if (file['name'].endsWith('.json') || file['name'].endsWith('.baja')) {
                                        ...
                                        buttons: [
                                            {
                                                label: 'Save', ionFunction: createIonFunction(async (button) => {
                                                    ...
                                                    let gs = await JSON.parse(JSON.stringify(graph, function (key, value) {
                                                        ...
                                                        } else if (key === 'selectedTradck' || key === 'selectedTrack') {
                                                            return null;
                                                        }
                                                        else
                                                    ...
                                                    db.saveScreen(libraryid, folderid, gs, name, (cstart, cend, total, fileid) => {
                                                        ...
                                                        if (progressBar) {
                                                            ...
                                                            if (progress === 100) {
                                                                setTimeout(() => {
                                                                    clear();
                                                                    exec('baja/manchester/open-screen', libraryid, fileid)
