    ...
    try {
        ...
        const chainPts = (cx, cy, step, anglesDeg) => {
            ...
            return pts;
                    ...
                    const r = bondLen;
                    const pts = hexPts(wx, wy, r, -30);
                    ringFromPts(Shape, Glyph, pts);
                        for (let i of [0, 2, 4]) { const p1 = pts[i]; const p2 = pts[(i+1)%6]; const dx = p2.x - p1.x; const dy = p2.y - p1.y; const len = Math.sqrt(dx*dx + dy*dy) || 1; const nx = -dy / len; const ny = dx / len; const inset = bondLen * 0.12; addBond(Shape, Glyph, p1.x + nx*inset, p1.y + ny*inset, p2.x + nx*inset, p2.y + ny*inset, { strokeWidth: 1.5 }); }

                    platetrack.wb(null);

                } catch (err) {
                    console.error('draw phenyl group failed:', err);
                    platetrack.wb(null);
                }
            },
