        ...
        const chainPts = (cx, cy, step, anglesDeg) => {
            ...
            return pts;
                    ...
                    ringFromPts(Shape, Glyph, pts);
                        addAtomLabel(Shape, Glyph, pts[0].x, pts[0].y, 'N');
                    addAtomLabel(Shape, Glyph, pts[3].x, pts[3].y, 'N');

                    platetrack.wb(null);

                } catch (err) {
                    console.error('draw piperazine ring failed:', err);
                    platetrack.wb(null);
                }
            },
