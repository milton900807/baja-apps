        ...
        const chainPts = (cx, cy, step, anglesDeg) => {
            ...
            return pts;
                    ...
                    addAtomLabel(Shape, Glyph, o.x, o.y, 'O');
                        const p = { x: wx + bondLen*2, y: wy + bondLen*0 }; addBond(Shape, Glyph, o.x, o.y, p.x, p.y); addAtomLabel(Shape, Glyph, p.x, p.y, 'OR');

                    platetrack.wb(null);

                } catch (err) {
                    console.error('draw ester group failed:', err);
                    platetrack.wb(null);
                }
            },
