    ...
    exec('baja/chem/biopolymer.js').then((Biopolymer) => {
        ...
        graph.addMouseDownListener(async (x, y) => {
            ...
            if (selected_chemistry == null) {
                graph.setMessage("Pleaes select chemistry ")
                return;
            }
            if (selected && startIndex >= 0 && endIndex > startIndex) {
                endIndex = startIndex + Biopolymer.getTemplateLength(selected_chemistry)
                complete = true;
                ...
                let width = (endIndex - startIndex) / 2;
                let xi = selectedTrack.markstart - selectedTrack.tgraph.xi
                let xf = selectedTrack.markend - selectedTrack.tgraph.xi

                console.log(xi + "<- xi, xf " + xf)
                                                        ...
                                                        buttons: [
                                                            {
                                                                label: 'Apply', ionFunction: createIonFunction(async () => {
                                                                    ...
                                                                    let xi = selectedTrack.markstart - selectedTrack.tgraph.xi
                                                                    let t = await selectedTrack.createSecondaryStructure(xi, sequence, name, new EngineMonitor((en) => {
