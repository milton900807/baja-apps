            ...
            'buttons': [
                    ...
                    x: 2, y: 0, label: 'Phase Sequence', ionFunction: createIonFunction(async () => {
                        if (graph.track.length > 0) {
                            ...
                            if (hasSnpindel == 1) {
                                ...
                                await exec('baja/manchester/annotation/variant-primer-probe-actions.js', graph, genegraph_panel_layout, true);
                            } else {
                                graph.setMessage('No variants found.')
                            }
                        }
...

                {
                    x: 3, y: 0, label: 'Add VCF', ionFunction: createIonFunction(async () => {
                        return new Promise(async (resolve, reject) => {
                            ...
                            await exec('baja/manchester/annotation/vcf-load.js', graph, library, folder, phasetarget);
                        });
                ...
                },
                {
                    x: 6, y: 0, label: 'Prepare Oligos', ionFunction: createIonFunction(async () => {
                        if (graph.track.length > 0) {
                            ...
                            if (hasOligos == 1) {
                                await exec('baja/manchester/annotation/send-oligos.js', graph).then(async (oligos) => {
                            ...
                            if (hasOligos == 1) {
                                ...
                                if (needsOfftarget) {
                                    ...
                                    let confirm = await exec('baja/lib/confirm.js', 'Some oligos do not have offtargets.  Continue?', async () => {
                                        await exec('baja/manchester/annotation/rule-application-wizard.js', library, folder, graph)
                                    })
                                    showModal(confirm)
                                } else
                                    await exec('baja/manchester/annotation/rule-application-wizard.js', library, folder, graph)
                ...
                },
                {
                    x: 5, y: 0, label: 'Upload Filtered Oligos', ionFunction: createIonFunction(async () => {
                        ...
                        await exec('baja/manchester/annotation/read-filtered-oligos.js', graph, library, folder);
                    })
                },
                {
