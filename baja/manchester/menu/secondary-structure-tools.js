        ...
        let list_buttons = [

            {
                x: 0, y: 0, label: 'New', ionFunction: createIonFunction(async () => {
                    await exec('baja/manchester/menu/draw-secondary-structure3.js', graph, genegraph_panel_layout);
                })
            ...
            },
            {
                x: 3, y: 0, label: 'Map', ionFunction: createIonFunction(async () => {
                    await exec('baja/manchester/menu/mouse-over-highlight-all.js', graph);
            ...
            if (foundIres_db) {
                list_buttons.push({
                    x: 5, y: 0, label: 'IRES', ionFunction: createIonFunction(async () => {
                        await exec('baja/manchester/menu/ires-search.js', graph);
