function (graph) {
    // Annotation tool: a free-text note box for longer comments.
    ...
    let PostItNote = null;
    exec('flexigraph/shapes/postit-note.js').then((k) => { PostItNote = k; });

    graph.clearMouseListeners();
    graph.setMouseMode("msg: Click and drag to add a note");
    graph.selectOff();
    let md = false;
    graph.addMouseDownListener((x, y) => {
        if (!PostItNote) return;   // class not loaded yet
        md = true;
        graph.currentShape = new PostItNote('test', x, y);
    });
