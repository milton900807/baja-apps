function (server, graph, genegraph_panel_layout, presetTrack) {
    ...
    return (async () => {
        ...
        restoreHover();
        // TEMPORARY DIAGNOSTIC — the whole context printed as JSON to the browser console
        // after the design finishes. Open the console and copy the block that follows the
        // "ALLELE-SELECTIVE DIAGNOSTIC" line. Remove once the allele question is settled.
        try {
            console.log('ALLELE-SELECTIVE DIAGNOSTIC\n' + JSON.stringify(dbgCtx, null, 2));
        } catch (e) { }
        return true;
    })();
}
