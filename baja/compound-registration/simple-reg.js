            ...
            try {
                ...
                for (let i = 0; i < oligos.length; i++) {
                    ...
                    if (!o.libID) {
                        ...
                        try {
                            let file = await client.api(`/drives/${libid}/root:/bajabio-xfiles/reg/${o.type}/${prefix}/${o.sequence}`).get();
                            if (file) {
                                console.log(" now we need to verify if the structyure is the same ")
                                file = await client.api(`/drives/${libid}/items/${file.id}`).get();
                                if (file) {
                                    graph.setMessage(" Already registered " + o.id)
                                }
                            }
                        } catch (exception) {
                            o.id = i + startIndex;
