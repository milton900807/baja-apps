    ...
    showWidget({
        ...
        exec('flexigraph/gene.js', progressBar).then(async (graph) => {
            ...
            let saveSaveScreen = async () => {
                let savedScreens = await exec('manchester/io/save-obj_light.js', graph, genegraph_panel_layout, path)
                showModal(savedScreens);
            }
            ...
            tools_menu = [
                ...
                },
                {
                    'label': 'ASO', 'ionfunction': createIonFunction(async () => {
                        ...
                        graph.showWindowMenu(await exec('baja/manchester/menu/load-chemistry-tools2', graph, genegraph_panel_layout), 10, 10, 400)
                    })
                },
...

                {
                    'label': 'Models', 'ionfunction': createIonFunction(async () => {
                        graph.showWindowMenu(await exec('baja/manchester/menu/splicing/splicing-tools4', graph, genegraph_panel_layout), 10, 10, 400)
                    })
                }
                ,
                {
