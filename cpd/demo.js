function (path, config) {
    ...
    showWidget({
        ...
        progressBar(20);
        let PlateTrack = await exec('baja/plate/plate-track.js', progressBar)
        ...
        exec('flexigraph/gene2plates.js', pm, progressBar).then(async (graph) => {
            ...
            let saveAsSaveScreen = async () => {
                await exec('screen/io/save-as-obj-tp.js', graph, genegraph_panel_layout, path)
            }
            let openWF = async () => {
            ...
            let publicPublish = async () => {
                ...
                if (pm.plateTrack && im) {
                    await exec('screen/io/save-as-obj-tp-public.js', graph, genegraph_panel_layout, path, '/app/cpd/editor', im)
                } else {
                    await exec('screen/io/save-as-obj-tp-public.js', graph, genegraph_panel_layout, path, '/app/cpd/editor')
                }
            }
            progressBar(80);
            ...
            if (!timesup) {
                ...
                const ai_create_file_items__dep = []

                ai_create_file_items.push({
                    'label': 'Therapeutics development cost analysis', click: (async () => {
                        ...
                        const txt = 'I want to create a drug for rett syndrome with a patient population of about 6000. ';
                        let initalText = true;
                        setTimeout(() => {
                            let i = 0;
                            let currentText = '';
                                    ...
                                    [
                                        {
                                            ...
                                            'component': {
                                                ...
                                                data: `
                                                    <H4>
                                                    <font color="navy">
                                                    Write a short couple of sentences about the therapeutic you want to create (Disease etc):
                                                    </font> </h4>   Click in the text box below to start.
                                                                            ...
                                                                            setTimeout(async () => {
                                                                                ...
                                                                                setTimeout(async () => {
                                                                                    ...
                                                                                    setTimeout(async () => {
                                                                                        ...
                                                                                        setTimeout(() => {
                                                                                            plate_graph.plateTrack.trimAssumptionRows()
                ...
                ai_create_file_items.push({
                    'label': 'bajabio Screening analysis', click: (async () => {
                        const plate_graph = pm;
                        const pt = pm.plateTrack;
                        let sequenceTextEditor;
                        let descHook = createIonFunction((p) => {
                                                    ...
                                                    buttons: [
                                                        ...
                                                        },
                                                        {
                                                            label: 'Build', ionFunction: createIonFunction(async () => {
                                                                ...
                                                                const user_prompt = content;
                                                                plate_graph.plateTrack.setMessage("Generating Assumptions...", 5)
                                                                let model = await exec('py/openai/assumptions-bajabio-screening.py', em, content)
                                                                exec('baja/draw/data-model-to-tables-gpt', plate_graph.plateTrack, model).then(async r => {
                                                                    plate_graph.plateTrack.setMessage(null)
                                                                    plate_graph.plateTrack.setMessage("These are the Assumptions! You will edit these.", 1)
                                                                    setTimeout(() => {
                                                                        ...
                                                                        setTimeout(async () => {
                                                                            ...
                                                                            plate_graph.plateTrack.setMessage('PnL', 5)
                                                                            let ts = (t.toValueFormulaJSON())
                                                                            let pnl = await exec('py/openai/budget-for-bajabio-screen.py', user_prompt, ts)
                                                                            let r = await exec('baja/draw/data-model-to-tables-gpt', plate_graph.plateTrack, pnl)
                ...
                ai_create_file_items__dep.push(
                    makeAiCreateItem('Quick Gantt', async () => {
                        let sequenceTextEditor;
                        let descHook = createIonFunction((p) => {
                            sequenceTextEditor = p;
                        });
                        ...
                        const txt = `
    Target Discovery Jan 15–May 15, 2026
    Mechanism Validation  June 16, 2026 through August 15, 2026
    Drug Candidate Screening 2026-08-16 to 2026-11-15
    Lead Identification and Validation — week of Nov 16, 2026
    In Vivo Toxicology Dec 2, 2026 thru Mar 3, 2027
    PK/PD (03-04-2027  04-08-2027)
    Large Animal Toxicology: Apr 9, 2027 until Aug 27, 2027`
                        let initalText = true;
                        let i = 0;
    ...
    <font color="navy">
                                                    ...
                                                    buttons: [
                                                        ...
                                                        },
                                                        {
                                                            ...
                                                            ionFunction: createIonFunction(async () => {
                                                                ...
                                                                plot.grid.rescale();
                                                                plot.distributeYValues()
                                                                pm.plateTrack.setPlotCenter(plot);

                                                                setTimeout(async () => {
                                                                    await plot.gotoCenterTime(pm.plateTrack)
                                                                }, 1000)
                ...
                file_items.push({
                    'label': 'New...', 'ionfunction': createIonFunction(async () => {
                        let confirm = await exec('baja/lib/confirm.js', 'Are you sure you want to delete all and start over?', async () => {
                            pm.plateTrack.reset('/app/cpd/demo');
            ...
            setTimeout(async () => {
                graph.isPreviousState().then(async r => {
                    if (r) {
                    }
                })
                if (wb)
                ...
                const aimenu = ai_create_file_items;
                graph.setMessage('')
                ...
                pm.tools = ai_create_file_items;

                let Menu = await exec('flexigraph/big-menu.js');
                const smenu2 = new Menu(
                    aimenu,
                    pm.plateTrack.grid.Xwc(pm.plateTrack.grid.xi + pm.plateTrack.grid.width / 2 - 200),
                    pm.plateTrack.grid.Ywc(pm.plateTrack.grid.yi + pm.plateTrack.grid.height / 2 - 20 * aimenu.length / 2),
                    'rgb(205, 255, 155)',
                    'black',
                    1
                );
                pm.plateTrack.showMenu(smenu2)
