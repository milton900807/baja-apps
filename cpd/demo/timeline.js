        ...
        exec('flexigraph/gene2plates.js', pm, progressBar).then(async (graph) => {
            ...
            if (path.endsWith('.bjb')) {
                ...
                if ((config != null && config.user != null) || path.startsWith('/myfiles/')) {
                    ...
                    if (rs.shared_from) {
                        ...
                        window.history.pushState({ 'rna-screen': path }, 'yak', `/app/cpd/baja-analytics?path=${path}`);
                        if (rs.msg) {
                            clear();
                            log(rs.msg + ' ' + p)
                            return;
            ...
            let publicPublish = async () => {
                ...
                if (pm.plateTrack && im) {
                    ...
                    await exec('screen/io/save-as-obj-tp-public.js', graph, genegraph_panel_layout, path, '/app/cpd/baja-analytics', im)
                } else {
                    await exec('screen/io/save-as-obj-tp-public.js', graph, genegraph_panel_layout, path, '/app/cpd/baja-analytics')
                }
            }
            progressBar(80);
...
<H4>
  <font color="navy">Write a short paragraph that describes the timeline you want to create. </font>
                ...
                ai_create_file_items.push({
                    'label': 'Profit-loss Model', 'ionfunction': createIonFunction(async () => {
                ...
                'component': {
                    ...
                    data: {
                        ...
                        menus: [
                            ...
                            {
                                'label': 'Time&Money', 'items': ai_create_file_items
                            },
                                                    ...
                                                    'label': 'Operations', 'items': [
                                                        {
                                                            label: 'Reset', ionfunction: createIonFunction(async () => {
                                                                await exec('baja/table/yakgen', path, config, '/app/cpd/baja-analytics')
                                                            })
                                                        },
                                                    ]
