function (path, config) {
    ...
    showWidget({
        ...
        let PlateManager = class PlateManager {
            ...
            app;
            constructor() {
                this.plateTrack = new PlateTrack();
        ...
        exec('flexigraph/gene2plates.js', pm, progressBar).then(async (graph) => {
            ...
            async function verifyUserPath(path) {
            ...
            if (path.endsWith('.bjb') || path.endsWith('.ljt')) {
                let host_ = window['env']['apiUrl']
                ...
                if ((config != null && config.user != null) || path.startsWith('/myfiles/')) {
                    ...
                    if (rs.shared_from) {
                        ...
                        window.history.pushState({ 'rna-screen': path }, 'yak', `/app/cpd/viewer?path=${path}`);
                        if (rs.msg) {
                            clear();
                            log(rs.msg + ' ' + p)
                            return;
            ...
            let openSaveScreen = async () => {
                let v = await exec('baja/table/io/open-yakro', graph, pm, '/app/cpd/viewer')
                showModal(v)
            }
            ...
            let publicPublish = async () => {
                ...
                if (pm.plateTrack && im) {
                    ...
                    await exec('screen/io/save-as-obj-tp-public.js', graph, genegraph_panel_layout, path, '/app/cpd/viewer', im)
                } else {
                    await exec('screen/io/save-as-obj-tp-public.js', graph, genegraph_panel_layout, path, '/app/cpd/viewer')
                }
            }
            progressBar(80);
            ...
            if (MSGraph.isLoggedIn()) {
                ...
                ai_create_file_items.push({
                    'label': 'Milestones', 'ionfunction': createIonFunction(async () => {
  ...
  <font color="navy">Write a short paragraph that describes the timeline you want to create. </font>
                                                            ...
                                                            label: 'Build timeline', ionFunction: createIonFunction(async () => {
                                                                ...
                                                                setTimeout(async () => {
                                                                    ...
                                                                    if (model && model.milestones) {
                                                                        ...
                                                                        const xMin = Math.min(...model.milestones.map(p => p.x));
                                                                        const xMax = Math.max(...model.milestones.map(p => p.x));
                ...
                ai_create_file_items.push({
                    label: 'Gantt Chart', ionfunction: createIonFunction(async () => {
                        ...
                        const txt = `  Create a timeline for drug discovery
                                                                            ...
                                                                            setTimeout(async () => {
                                                                                ...
                                                                                setTimeout(async () => {
                                                                                    ...
                                                                                    setTimeout(async () => {
                                                                                        ...
                                                                                        let model4 = await exec('py/openai/find-waterfall-plot-wells.py', ls)
                ...
                file_items.push({
                    'label': 'New...', 'ionfunction': createIonFunction(async () => {
                        let confirm = await exec('baja/lib/confirm.js', 'Are you sure you want to delete all and start over?', async () => {
                            pm.plateTrack.reset('/app/cpd/viewer');
