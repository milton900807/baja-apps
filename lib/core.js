...
async function verifyUserPath(path, app, position) {
    ...
    //
    // FAIL-OPEN is deliberate and is a change from the old behavior. Previously a network error
...
let CurrentLayout = class CurrentLayout {
    ...
    static setComponent(label, compObj) {
        ...
        //
        // With nothing stashed this falls through to the original behavior, so a host that
...
// immediately (does not wait for the close, matching the prior behavior).
