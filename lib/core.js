...
async function verifyUserPath(path, app, position) {
    // Does this user have access to `app`?
    //
    // This used to begin with `if (window.env.auth !== 'b2c') return { allowed: true }`, which
    // conflated the deployment's AUTH MODE with the user's ENTITLEMENT: a deployment not running
    // b2c never asked the question at all and let everyone through, while under b2c a licence
    // file that did not say 'granted' could deny someone with a perfectly good subscription.
    // Neither is a fact about the account. Entitlement is now decided by the two systems that
    // actually hold it, in this order:
    //
    //   1. the LICENCE FILE (/verify-user)  -- site licences, seats granted out of band
    //   2. the SUBSCRIPTION (Stripe)        -- someone who pays is entitled, licence or not
    //
    // FAIL-OPEN is deliberate and is a change from the old behaviour. Previously a network error
    // talking to /verify-user returned allowed:false, so an outage locked out every user of
    // every app -- including paying ones. Access is now denied ONLY on a definitive negative:
    // the licence says not granted AND Stripe says the subscription is not active. "I could not
    // find out" is never treated as "no".
    //
    // A client-side check is advisory by nature -- anyone can edit the answer in devtools -- so
    // this decides which UI to present. Anything that costs money stays enforced server-side.
    var resolvedPath = path;
    if (!position) {
        position = 'all'
    }
    if (!window.env) {
        return { allowed: true, path: resolvedPath, reason: 'No environment configured; not gating.' };
    }

    var payload = {
        email: (typeof getUser === 'function' ? getUser() : ''),
        app: app,
        position: position
    };
    ...
    var rs = null;
    var licenceReachable = true;

    try {
        rs = await POSTJSON(payload, host + '/verify-user');
    } catch (e) {
        // Unknown, not denied -- see the fail-open note above.
        console.warn('verifyUserPath: /verify-user unreachable; falling back to the subscription', e);
        licenceReachable = false;
    }
    ...
    if (licenseStatus !== 'granted') {
        // The licence did not grant it (or could not be read). Ask Stripe, which is the system
        // that knows whether this account pays.
        //   true  -> entitled
        //   false -> definitively not subscribed
        //   null  -> could not verify (no email / Stripe not configured / network)
        var subActive = null;
        try {
            var SUB = await exec('lib/subscription.js');
            subActive = await SUB.checkSubscription();
        } catch (e) {
            console.warn('verifyUserPath: subscription check failed', e);
            subActive = null;
        }
        ...
        if (subActive === false) {
            ...
            return {
                ...
                coreStatus: coreStatus,
                licenseStatus: licenseStatus,
                reason: reason || 'You do not have a valid license or active subscription for this application.',
                raw: rs
            };
        }
        ...
        if (subActive === true) {
            ...
            coreStatus = coreStatus || 'active';
            reason = reason || 'Access granted via active subscription.';
        } else {
            // Neither system could answer. Let the user through and say so, rather than locking
            // out an entire deployment because one endpoint is down.
            licenseStatus = licenseStatus || 'unknown';
            coreStatus = coreStatus || 'unknown';
            reason = reason || (licenceReachable
                ? 'Entitlement could not be verified; access allowed.'
                : 'License service unreachable; access allowed.');
        }
    }
...
// Focus a text field -- unless this is a phone or a tablet.
//
// On a touch device, focusing an input is not free: it opens the on-screen keyboard, which
// covers between a third and a half of the window that just opened. Nobody asked to type
// yet -- they opened a panel to READ it -- so the first thing they have to do is dismiss a
// keyboard to see the thing they opened. On a search box it is worse than useless, because
// the list it filters is the part now hidden.
//
// Desktop keeps it. There a focused field costs nothing and saves a click.
//
// One helper rather than the same isMobile() check at every window, because the ones that
// get missed are the ones nobody opens on a phone until a user does.
function focusUnlessMobile(el, delayMs) {
    try {
        if (!el || typeof el.focus !== 'function') return false;
        if (typeof isMobile === 'function' && isMobile()) return false;
        if (delayMs) { setTimeout(() => { try { el.focus(); } catch (e) { } }, delayMs); return true; }
        el.focus();
        return true;
...
const isMobile = () => {
    ...
    try {
        ...
        // Classic mobile user-agents (also matches generic "Mobile" tokens).
        if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|Windows Phone|Mobile/i.test(ua)) {
            return true;
        }
        // iPadOS 13+ reports a DESKTOP "Macintosh" UA — distinguish it by touch support.
        if (/Macintosh/i.test(ua) && (navigator.maxTouchPoints || 0) > 1) {
            return true;
        }
        // Touch-primary device with a phone/tablet-sized viewport. Also catches browser
        // device-emulation (DevTools) that sets a coarse pointer without spoofing the UA.
        // A desktop with a mouse reports `(pointer: fine)`, so it stays on the desktop UI.
...
let CurrentLayout = class CurrentLayout {
    ...
    // Panel objects that MEAN "put the editor back". A host registers its editor panel once
    // with registerMainRestoreAlias(); see setComponent below for why.
    static registerMainRestoreAlias(comp) {
        if (!comp) return;
        const a = CurrentLayout.__mainRestoreAliases;
        if (a.indexOf(comp) < 0) a.push(comp);
    }

    static setComponent(label, compObj) {
        // Roughly 460 call sites across the lionscript library return to the editor with
        //     CurrentLayout.clearComponent('mainPanel');
        //     CurrentLayout.setComponent('mainPanel', genegraph_panel_layout);
        // which mounts the panel object the tool was HANDED rather than the layout the editor
        // actually stashed for mainPanel. The clear empties the panel and the editor canvas
        // never comes back. The correct call is reset('mainPanel'), which remounts the stashed
        // layout and -- through the patch manchester/editor.js installs on reset -- re-arms
        // mouse-over-highlight, something a hand-rolled restore skips entirely.
        //
        // Fixing that by editing all 460 sites would be 460 chances to break a working panel,
        // and would still miss any site where the variable is named something else. Instead the
        // restore is recognised HERE, by OBJECT IDENTITY: the host registers its editor panel
        // once, and only that exact object is rerouted. A tool mounting its OWN panel into
        // mainPanel -- sequence_input, paint_panel, bwpanel and the rest -- is untouched,
        // which is what keeps this from hijacking legitimate mounts.
        //
        // With nothing stashed this falls through to the original behaviour, so a host that
        // never stashed a mainPanel is exactly as it was.
        if (label === 'mainPanel' && !CurrentLayout.__restoring
            && CurrentLayout.__mainRestoreAliases.indexOf(compObj) >= 0
            && lion_engine.getStashed('mainPanel')) {
            // reset() calls back into setComponent with the STASHED layout, which is a
            // different object and so passes straight through. The flag is belt and braces
            // against a host that registers the stashed layout as its own alias.
            CurrentLayout.__restoring = true;
            try { CurrentLayout.reset('mainPanel'); } finally { CurrentLayout.__restoring = false; }
            return;
        }
...
// Assigned here rather than as static class fields: the rest of this file predates them and
// the minifier is only trusted with plain assignments.
CurrentLayout.__mainRestoreAliases = [];
CurrentLayout.__restoring = false;
...
// Shared "demo panel" look for the DOM-overlay prompts (navy card, rounded, soft shadow).
function _bajaPromptPanel(id, width) {
    try { const prev = document.getElementById(id); if (prev && prev.parentNode) prev.parentNode.removeChild(prev); } catch (e) { }
    const wrap = document.createElement('div');
    wrap.id = id;
    const wCss = (typeof width === 'number' && width > 0) ? ('min(' + Math.round(width) + 'px,92vw)') : 'min(520px,92vw)';
    wrap.style.cssText = 'position:fixed;top:60px;left:50%;transform:translateX(-50%);z-index:2147483000;width:' + wCss + ';'
        + 'max-height:86vh;overflow:auto;background:#0b2545;color:#fff;border-radius:12px;box-shadow:0 12px 40px rgba(0,0,0,0.45);'
        + 'border:1px solid rgba(255,255,255,0.14);font:13px system-ui,-apple-system,Roboto,Arial,sans-serif;padding:16px;';
    return wrap;
}
function _bajaPromptBtn(label, kind) {
    const b = document.createElement('button');
    b.textContent = label;
    b.style.cssText = (kind === 'primary')
        ? 'background:#22c55e;color:#06230f;font-weight:700;border:none;border-radius:999px;padding:7px 20px;cursor:pointer;'
        : 'background:transparent;color:#cbd5e1;border:1px solid rgba(255,255,255,0.25);border-radius:999px;padding:7px 16px;cursor:pointer;';
    return b;
}

// prompt(): DOM overlay (demo-panel style) with a field per input label. Resolves { label: value }
// on OK, or undefined on Cancel. `txt` is HTML.
function prompt(txt, inputs, default_values, width, height) {
    return new Promise((resolve) => {
        try {
            inputs = inputs || [];
            default_values = default_values || [];
            const wrap = _bajaPromptPanel('baja-prompt-panel', width);

            const head = document.createElement('div');
            head.style.cssText = 'font-weight:700;margin-bottom:8px;';
            head.innerHTML = '' + (txt == null ? '' : txt);
            wrap.appendChild(head);

            // An input is a label string, or an object { label, multiline, rows, placeholder }
            // for a multi-line field (a textarea). Results are keyed by the label either way.
            const fieldEls = [];
            inputs.forEach((spec, i) => {
                const isObj = spec && typeof spec === 'object';
                const lab = isObj ? ('' + (spec.label || ('Field ' + (i + 1)))) : ('' + spec);
                const l = document.createElement('label');
                l.textContent = lab;
                l.style.cssText = 'display:block;margin:10px 0 4px;opacity:.85;';
                wrap.appendChild(l);
                const multiline = !!(isObj && spec.multiline);
                const inp = document.createElement(multiline ? 'textarea' : 'input');
                if (multiline) { inp.rows = (isObj && spec.rows) || 3; inp.style.resize = 'vertical'; }
                if (isObj && spec.placeholder) inp.placeholder = '' + spec.placeholder;
                const dv = (default_values && !Array.isArray(default_values) && (lab in default_values)) ? default_values[lab] : default_values[i];
                inp.value = (dv != null) ? ('' + dv) : '';
                inp.style.cssText += ';width:100%;box-sizing:border-box;background:#0a1e3a;color:#e8eef6;'
                    + 'border:1px solid rgba(255,255,255,0.18);border-radius:8px;padding:8px 10px;font:13px system-ui,Arial;';
                inp.__label = lab;
                inp.__multiline = multiline;
                wrap.appendChild(inp);
                fieldEls.push(inp);
            });

            // What happens inside the panel stays inside the panel. The editor listens on the
            // window for paste (an ENST id, a FASTA, a whole graph pasted onto the canvas) and
            // for keys; text pasted or typed into a prompt field must never reach those, or a
            // pasted description becomes tracks on the canvas. Stopped here, in the bubble
            // phase, after the field itself has received the event and done its own paste.
            for (const ev of ['paste', 'cut', 'copy', 'keydown', 'keyup', 'keypress', 'input']) {
            ...
            const cancel = _bajaPromptBtn('Cancel', 'ghost');
            const ok = _bajaPromptBtn('OK', 'primary');
            row.appendChild(cancel); row.appendChild(ok);
            wrap.appendChild(row);

            // On a phone a multi-line prompt takes the whole screen: the panel becomes a
            // full-height column and the textarea takes every pixel between the title and
            // the buttons. 16px text, because iOS zooms the page for anything smaller when a
            // field takes focus, and that zoom is what makes a small panel unusable there.
            let mobileFull = false;
            try { mobileFull = fieldEls.some((f) => f.__multiline) && typeof isMobile === 'function' && isMobile(); } catch (e) { mobileFull = false; }
            if (mobileFull) {
                wrap.style.cssText += ';top:0;left:0;right:0;bottom:0;width:100vw;max-width:100vw;height:100vh;height:100dvh;max-height:none;'
                    + 'transform:none;border-radius:0;display:flex;flex-direction:column;padding:14px;';
                for (const f of fieldEls) {
                    if (f.__multiline) f.style.cssText += ';flex:1 1 auto;min-height:40vh;font-size:16px;resize:none;';
                    else f.style.cssText += ';font-size:16px;';
                }
                row.style.cssText += ';flex:0 0 auto;margin-top:12px;';
                for (const b of [cancel, ok]) { try { b.style.cssText += ';flex:1 1 auto;padding:12px 16px;font-size:16px;'; } catch (e) { } }
            }

            document.body.appendChild(wrap);
            promptVisible = true;
            const close = () => { try { if (wrap.parentNode) wrap.parentNode.removeChild(wrap); } catch (e) { } promptVisible = false; };
            cancel.onclick = () => { close(); resolve(); };
            ok.onclick = () => {
                const results = {};
                for (const inp of fieldEls) {
                    const lab = inp.__label;
                    let raw = inp.value;
                    if (lab === 'Name') raw = ('' + raw).replace(/-/g, '_').replace(/\s+/g, '').replace(/[^a-zA-Z0-9_]/g, '');
                    else if (typeof raw === 'string') raw = raw.trim();
                    results[lab] = raw;
                }
                close(); resolve(results);
            };
            // Enter submits a single-line field; in a textarea Enter is a newline and
            // Ctrl/Cmd+Enter submits. Escape cancels either way.
            wrap.addEventListener('keydown', (e) => {
                const inTextarea = !!(e.target && e.target.__multiline);
                if (e.key === 'Enter' && (!inTextarea || e.ctrlKey || e.metaKey)) { try { e.preventDefault(); } catch (e2) { } ok.click(); }
                else if (e.key === 'Escape') cancel.click();
            });
            if (fieldEls[0]) focusUnlessMobile(fieldEls[0]);
        } catch (e) { resolve(); }
    });
}

// infoPrompt(): DOM overlay (demo-panel style) showing HTML `txt` with a Close button. Resolves
// immediately (does not wait for the close, matching the prior behaviour).
function infoPrompt(txt, width, height) {
    return new Promise((resolve) => {
        try {
            const wrap = _bajaPromptPanel('baja-info-panel', width || 460);
            const body = document.createElement('div');
            body.style.cssText = 'margin-bottom:12px;';
            body.innerHTML = '' + (txt == null ? '' : txt);
            wrap.appendChild(body);
            const row = document.createElement('div');
            row.style.cssText = 'display:flex;justify-content:flex-end;';
            const closeBtn = _bajaPromptBtn('Close', 'primary');
            row.appendChild(closeBtn);
            wrap.appendChild(row);
            document.body.appendChild(wrap);
            const close = () => { try { if (wrap.parentNode) wrap.parentNode.removeChild(wrap); } catch (e) { } };
            closeBtn.onclick = close;
            wrap.addEventListener('keydown', (e) => { if (e.key === 'Escape' || e.key === 'Enter') close(); });
        } catch (e) { }
        resolve();
    });
}
...
function showModal(obj, width, height) {
    mouse_down = false;
    // A null/undefined obj means "nothing to show" — e.g. confirm.js now renders its
    // confirmation on the graph canvas and returns null. Guard so showModal(null) is a
    // harmless no-op instead of throwing on obj.width.
    if (!obj) return;
    if (!width) {
        if (obj.width)
            width = obj.width;
...
// The free-tier allowance is enforced server-side (freeGate in baja-server), which answers
// HTTP 402 with {error:'free-limit', metric, used, limit, resetsOn, message} instead of the
// result. FunctionUtil.POSTJSON resolves rather than rejects on a non-2xx, handing back
// Angular's HttpErrorResponse, so the payload arrives nested under .error -- callers that
// only looked at the result shape saw "no results" and carried on as if nothing happened.
// Returns the payload when a call was refused for allowance, otherwise null.
function freeLimitInfo(r) {
    try {
        if (!r) return null;
        if (r.error === 'free-limit') return r;                       // the body itself
        const b = r.error;                                            // HttpErrorResponse
        if (b && b.error === 'free-limit') return b;
        if (+r.status === 402 && b) return b;                         // 402 in another shape
    } catch (e) { }
    return null;
}

// Species from an Ensembl stable id. Ensembl encodes the organism in the prefix -- ENST is
// human, and every other species carries a three-letter code before the feature letter:
// ENSMUST (mouse), ENSRNOT (rat), ENSCAFT (dog), ENSMFAT (cynomolgus), ENSMMUT (rhesus).
// Tracks were being labelled Human whatever they were, because the local /transcript endpoint
// returns GTF records with no species field and the loader filled the gap with a constant.
//
// Returns '' for anything it cannot read -- a RefSeq accession (NM_/XM_) does not encode the
// organism, and guessing there would put the wrong label back for a different reason.
// YEAST HAS NO PREFIX. Ensembl's S. cerevisiae ids are SGD's own: a systematic ORF name
// (YAL069W, YBL100W-A, Q0010 for the mitochondrion) or a named RNA (snR19, tP(UGG)A), with
// the transcript type hung on the end -- YAL069W_mRNA, snR19_snRNA, tP(UGG)A_tRNA -- and the
// transposable elements carry no suffix at all (YPR158W-B). Nothing in that says "yeast"
// the way ENST says human, so the shape itself is the tell.
const YEAST_TRANSCRIPT_RE = /^(?:[A-Za-z0-9()'-]+_(?:mRNA|ncRNA|tRNA|rRNA|snRNA|snoRNA|transcript)|Y[A-P][LR]\d{3}[WC](?:-[A-Z])?|Q\d{4})$/;
function isYeastTranscriptId(id) {
    return YEAST_TRANSCRIPT_RE.test(('' + (id == null ? '' : id)).trim());
}

function speciesFromTranscriptId(id) {
    const s = ('' + (id == null ? '' : id)).trim().toUpperCase();
    if (isYeastTranscriptId(('' + (id == null ? '' : id)).trim())) return 'Yeast';
    const m = s.match(/^ENS([A-Z]*?)[TGPER]\d/);
    if (!m) return '';
    return {
        '': 'Human',
        MUS: 'Mouse', RNO: 'Rat', CAF: 'Dog',
        MFA: 'Cynomolgus monkey', MMU: 'Rhesus macaque', PTR: 'Chimpanzee',
        SSC: 'Pig', OCU: 'Rabbit', CGR: 'Chinese hamster', BTA: 'Cow',
        OAR: 'Sheep', FCA: 'Cat', CPO: 'Guinea pig',
        GAL: 'Chicken', DAR: 'Zebrafish', CEL: 'C. elegans', DME: 'Fruit fly'
    }[m[1]] || '';
}
...
function getUser() {
    ...
    // Fall back to the OIDC session identity (Google / Facebook / Microsoft via /login),
    // so profile & email work for users who signed in through a non-Microsoft provider.
    try {
        let s = JSON.parse(localStorage.getItem('oidc.session') || 'null');
        if (s && s.user) return s.user.email || s.user.name || t;
    } catch (e) { /* ignore */ }
    return t;
}

// The signed-in OIDC user object ({ provider, email, name, picture, sub }) or null.
