...
async function verifyUserPath(path, app, position) {
    ...
    if (licenseStatus !== 'granted') {
        // Fall back to the Stripe subscription: an active subscription is itself a valid
        // access check, even when the file-based license isn't 'granted'.
        var subActive = false;
        try {
            var SUB = await exec('lib/subscription.js');
            subActive = (await SUB.checkSubscription()) === true;
        } catch (e) {
        ...
        // Active Stripe subscription → treat as granted.
        console.log('verifyUserPath: granted via active Stripe subscription');
