...
async function verifyUserPath(path, app, position) {
    ...
    // No client-side gating. This used to ask Stripe when the licence file did not say
    // 'granted' and return allowed:false on a definitive "not subscribed", which dropped the
    // apps into free mode or showed the checkout. The answer is always "allowed" now; the
    // licence and subscription status still ride along for anything that wants to show them,
    // and everything that costs money stays enforced server-side.
    if (licenseStatus !== 'granted') {
        licenseStatus = licenseStatus || 'unknown';
        coreStatus = coreStatus || 'unknown';
        reason = reason || (licenceReachable ? 'Access allowed; entitlement not gated in the client.' : 'License service unreachable; access allowed.');
    }
