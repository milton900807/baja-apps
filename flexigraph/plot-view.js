function (MGrid) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        function formatTimeLabel(x, xMin, xMax, start, end) {
            ...
            if (totalTimeRange > 10 * 365 * 24 * 60 * 60 * 1000) {
                return date.getFullYear().toString();
            }

            if (totalTimeRange > 60 * 24 * 60 * 60 * 1000) {
                return `${date.toLocaleString('default', { month: 'short' })} ${date.getFullYear()}`;
            }

            if (totalTimeRange > 24 * 60 * 60 * 1000) {
                return `${date.getMonth() + 1}/${date.getDate()}`;
            }

            if (totalTimeRange > 60 * 60 * 1000) {
        ...
        let MPlot = class MPlot {
            ...
            displayContextSpecificMenuItems(pt) {
                const m = []
                if (this.type === timeline)
                    this.buildTimelineMenu(pt, m);
                else
                        ...
                        click: async (scx, scy) => {
                            ...
                            let lasso = {
                                ...
                                mouseMoveListener: (x, y) => {
                                    scx_ = pt.grid.X(x);
                                    scy_ = y - 10;
                                    let tx = (this.grid.Xwc(scx_ - this.grid.xi * 2))
                                    this.__scx_ = x;
                                    this.__scy_ = y;
                                    this.__date = formatTimeLabel(tx, this.grid.xmin, this.grid.xmax, this.startDate, this.endDate)
                                },
                    ...
                    if (this.type && this.type.startsWith('bar')) {
                        ...
                        timelinePoints.forEach(point => {
                            ...
                            if (point.type === 'progress') {
                                ...
                                let filenameY = labelY - 16;

                                ctx.font = '14px Arial';
                                const nameWidth = ctx.measureText(point.name).width;
                                const nameHeight = 14;
                                const nameBox = { x: x - nameWidth / 2, y: filenameY, w: nameWidth, h: nameHeight * 2 };
                                ...
                                while (collision) {
                                    ...
                                    for (const label of previousLabels) {
                                        const overlap = !(nameBox.x + nameBox.w < label.x ||
                                            nameBox.x > label.x + label.w ||
                                            nameBox.y + nameBox.h + verticalOffset < label.y ||
                                            nameBox.y + verticalOffset > label.y + label.h);
                                        if (overlap) {
                                            verticalOffset -= nameHeight + 4;
                                            collision = true;
                                            break;
                                        }
                                    }
                                ...
                                if (point.filename) {
                                    ctx.font = '12px Arial';
                                    ctx.fillText(`${point.filename}`, x, filenameY);
                                }
                            }
            ...
            drawXAxisTimeTicks(ctx, _grid, minVal, maxVal, showQuarters = true) {
                ...
                const showMonths = pxPerMonth >= 10;
                const showDays = pxPerDay >= 10;
                const showHours = pxPerHour >= 15;

                const viewStartTime = startDate.getTime() + minVal * hourToMs;
                const viewEndTime = startDate.getTime() + maxVal * hourToMs;

                const viewStartDate = new Date(viewStartTime);
                const viewEndDate = new Date(viewEndTime);

                for (let year = viewStartDate.getFullYear(); year <= viewEndDate.getFullYear(); year++) {
                    for (let m = 0; m < 12; m++) {
                        const date = new Date(year, m);
                        const ms = date.getTime();
                        if (ms < viewStartTime || ms > viewEndTime) continue;
                        ...
                        ctx.moveTo(position, cymin);
                        ctx.lineTo(position, cymin + tickLength);
                        ctx.strokeStyle = isMajor ? '#000' : '#999';
                        ctx.stroke();

                        const textY = cymin + tickLength + 4;
                        ...
                        if (isMajor) {
                            ...
                            if (showQuarters && isQuarter) {
                                ...
                                ctx.font = '10px Arial';
                                ctx.fillStyle = '#666';
                                ctx.fillText(quarterLabel, position, textY + 14);
                            }
                            ...
                            ctx.textBaseline = 'top';
                            ctx.fillStyle = '#666';
                            ctx.fillText(quarterLabel, position, textY);
                        }
                    }
                }

                if (showDays || showHours) {
                    let curTime = new Date(viewStartTime);
                    curTime.setMinutes(0, 0, 0);

                    while (curTime.getTime() <= viewEndTime) {
                        const isHourTick = showHours;
                        const isDayTick = showDays && curTime.getHours() === 0;

                        if (isDayTick || isHourTick) {
                            const hourOffset = (curTime.getTime() - startDate.getTime()) / hourToMs;
                            const position = _grid.X(hourOffset);

                            ctx.beginPath();
                            ctx.moveTo(position, cymin);
                            ctx.lineTo(position, cymin + (isDayTick ? 5 : 3));
                            ctx.strokeStyle = isDayTick ? '#777' : '#aaa';
                            ctx.stroke();

                            if (isDayTick) {
                                const label = `${curTime.getDate()}`;
                                ctx.font = '10px Arial';
                                ctx.textAlign = 'center';
                                ctx.textBaseline = 'top';
                                ctx.fillStyle = '#555';
                                ctx.fillText(label, position, cymin + 10);
                            } else if (isHourTick) {
                        ...
                        curTime = new Date(curTime.getTime() + hourToMs);
                    }
                }
