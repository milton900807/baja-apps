function () {
    ...
    exec('flexigraph/gene.js').then(async (graph) => {
        ...
        let lib_id, file_id;
        let v = await CONSTANTS('lj/var.js')
        lib_id = v.LIB_ID;
        file_id = v.FILE_ID;
        ...
        window.history.pushState({ libid: lib_id }, 'Screen', `app/lj/init?fi=${file_id}`);
                ...
                'buttons': [
                    ...
                    },
                    {
                        x: 8, y: 0, label: 'Zoom out', ionFunction: createIonFunction(() => {
                            ...
                            graph.zoomXY(graph.getxmin() - l, graph.getxmax() + l, graph.getymin() - ly, graph.getymax() + ly);
                    ...
                    },
                    {
                        ...
                        }), icon: '/assets/img/icons/png/expand-up.png'
                    },
                    {
                        ...
                        }), icon: '/assets/img/icons/png/expand-down.png'
                    },

                    {
                        x: 16, y: 0, label: 'resize x', ionFunction: createIonFunction(() => {
                            let l = (graph.getxmax() - graph.getxmin()) / 4;
                            graph.zoom(graph.getxmin() - l, graph.getxmax() + l);
                        }), icon: '/assets/img/icons/png/contract-x.png'
                    },
                    {
                        x: 18, y: 0, label: 'expand down', ionFunction: createIonFunction(() => {
                            let l = (graph.getxmax() - graph.getxmin()) / 4;
                            graph.zoom(graph.getxmin() + l, graph.getxmax() - l);
                        }), icon: '/assets/img/icons/png/expand-x.png'
        ...
        let display = {
            ...
            data: { ionFunction: createIonFunction(() => { return ` Selected chemistry template: ` + molecule_type_html_render(graph.props.selected_chemistry) }) }
        }
                                    ...
                                    menus: [
                                        ...
                                        screenActions,
                                        {
                                            'label': 'Chemistry', 'items': [
                                                ...
                                                {
                                                    'label': 'Select Template', 'ionfunction': createIonFunction(async () => {
